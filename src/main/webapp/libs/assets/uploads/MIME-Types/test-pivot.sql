...
INNER JOIN el_user_profile_data AS a1 ON (m.uid=a1.uid AND a1.field=\'controller\')
...
SELECT uid, MAX( IF( b =  \'controller\', val, NULL ) ) login, MAX( IF( b =  \'email\', val, NULL ) ) email, MAX( IF( b =  \'f_name\', val, NULL ) ) f_name, MAX( IF( b =  \'mtime\', val, NULL ) ) mtime
FROM (
...
SELECT uid,
MAX(IF(field=\'controller\',  value, NULL)) login,
