...
<input id="parentName" hidden value="${parentName}">
<input id="studentName" hidden value="${studentName}">
<input id="loginStuListSize" hidden value="${loginStuListSize}">
...
//        studentCharts('<%=request.getContextPath()%>');
    ...
    function setlogohello() {
        $("#hello").html('<h6><a><b>' + $("#parentName").val() + '</b>&nbsp;&nbsp;家长，欢迎您！</a></h6>');
        if( null !=$("#studentName").val() && $("#studentName").val()!="" ){
            $("#thisdapt").html('<a>当前查看学生：</a>');
            $("#deptvalue").html('<h6><b>' + $("#studentName").val() + '</b></h6>');
        }
    }
    ...
    function changeStudent() {
        $("#dialog").load("<%=request.getContextPath()%>/core/parent/toChangeStudent");
        $("#dialog").modal("show");
    }
    ...
    function chengeParentEdit() {
        $("#dialog").load("<%=request.getContextPath()%>/core/parent/toParentEdit?ifIndex=1");
        $("#dialog").modal("show");
    }
