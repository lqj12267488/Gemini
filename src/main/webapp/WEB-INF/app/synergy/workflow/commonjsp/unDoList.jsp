...
<script type="text/javascript">
    ...
    function pulldownRefresh() {
        ...
        setTimeout(function() {
            ...
            $.ajax({
                type: "POST",
                url: "<%=request.getContextPath()%>/workflowApp/result/getWorkFlowNameDownRefresh",
            ...
            function createEmlist(jsonStr){
                ...
                $.each(emlistData, function(key, map){
                    ...
                    li.innerHTML = '<div class="mui-table" >'+
                        ...
                        '<a open-type="common"  href="<%=request.getContextPath()%>/workFlowApp/result/listUnDoMenmbers?workFlowName='+map.workFlowName+'&type='+etype+'" >'
                        +workFlowNameData+'</a>' +
                        '</h3>' +
                        '</div></div>';
    ...
    function pullupRefresh() {
        setTimeout(function() {
            ...
            if(page == 1){
                ...
                $.ajax({
                    type: "POST",
                    url: "<%=request.getContextPath()%>/workflowApp/result/getWorkFlowNameUpRefresh",
                    data:{"page":page,"type":etype},
                    async: false,
                    dataType: 'json',
                    success: function(jsonData){
            ...
            function createEmlist(jsonStr){
                ...
                $.each(emlistData, function(key, map){
                    ...
                    li.innerHTML = '<div class="mui-table" >'+
                        '<div class="mui-table-cell mui-col-xs-10">'+
                        '<h3 class="mui-ellipsis">' +
                                '<a open-type="common"  href="<%=request.getContextPath()%>/workFlowApp/result/listUnDoMenmbers?workFlowName='+workFlowNameData+'&type='+etype+'" >'
                                            +workFlowNameData+'</a>' +
                        '</h3>' +
                        '</div></div>';
                    table.appendChild(li);
