...
<script type="text/javascript">
    ...
    function pulldownRefresh() {
        ...
        setTimeout(function() {
            ...
            $.ajax({
                type: "POST",
                url: "<%=request.getContextPath()%>/notice/getMessageName",
                data:{"page":page},
                async: false,
                dataType: 'json',
            ...
            function createEmlist(jsonStr){
                ...
                $.each(emlistData, function(key, map){
                    ...
                    var  flag = decodeURI(decodeURI(map.flag));
                    flag = decodeURI(decodeURI(flag));
                    ...
                    li.innerHTML = '<div class="mui-table" >'+
                        ...
                        '<a open-type="common"  href="<%=request.getContextPath()%>/notice/messageInfo?title='+map.title+'&flag='+flag+'&id='+map.id+'&type='+map.type+'&businessId='+map.businessId+'" >'
                        +title+'</a>' +
                        '</h3>' +
                        '<div style="font-size: 12px;color: #797979">'+"发布日期："+map.createTime+'</div>'+
                        '</div></div>';
            ...
            function createEmlist(jsonStr){
                ...
                $.each(emlistData, function(key, map){
                    ...
                    li.innerHTML = '<div class="mui-table" >'+
                        ...
                        +title+'</a>' +
                        '</h3>' +
                        '<div style="font-size: 12px;color: #797979">'+"发布日期："+publicTime+'</div>'+
                        '</div></div>';
                    table.appendChild(li);
                });
            }
    ...
    function turnFilesUpload(e) {
        ...
        $('#dialogFile').load('<%=request.getContextPath()%>/files/filesUpload?businessId=' + e + '&businessType=TEST&tableName=T_ZW_GOODSPURCHASING_WF');
