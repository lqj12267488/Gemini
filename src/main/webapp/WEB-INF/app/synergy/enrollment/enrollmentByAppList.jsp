...
<div class="mui-content">
    <header class="mui-bar mui-bar-nav">
        ...
        <h1 id="title" class="mui-title">新生登记列表</h1>
    </header>
    <div class=""></div>
    <div id="pullrefresh" class="mui-scroll-wrapper">
        <div class="mui-scroll">
            <!--数据列表-->
            <center>
                <button class="mui-btn mui-btn-primary" id="save" style="text-align:left;display: block; "
                        onclick="addEnrollmentStudent()">新生登记
                </button>
            </center>
...
<script type="text/javascript">
    ...
    function pulldownRefresh() {
        ...
        setTimeout(function() {
            ...
            $.ajax({
                type: "POST",
                url: "<%=request.getContextPath()%>/enrollmentApp/getEnrollmentByAppList",
                data:{"page":page},
                async: false,
                dataType: 'json',
            ...
            function createEmlist(jsonStr){
                ...
                $.each(emlistData, function(key, map){
                    var  majorShow = decodeURI(decodeURI(map.majorShow));
                    var  name = decodeURI(decodeURI(map.name));
                    var  idcard = decodeURI(decodeURI(map.idcard));
                    majorShow = decodeURI(decodeURI(majorShow));
                    ...
                    li.innerHTML = '<div class="mui-table" >'+
                        ...
                        '<a open-type="common"  href="<%=request.getContextPath()%>/enrollmentApp/enrollmentStudentDetail?studentId='+map.idcard+'" >'+name+','+idcard+'</a>' +
                        '</h3>' +
                        '<div style="font-size: 12px;color: #797979">'+"专业："+map.majorShow+'</div>'+
                        '</div></div>';
            ...
            function createEmlist(jsonStr){
                ...
                $.each(emlistData, function(key, map){
                    ...
                    li.innerHTML = '<div class="mui-table" >'+
                        ...
                        '<a open-type="common"  href="<%=request.getContextPath()%>/enrollmentApp/enrollmentStudentDetail?studentId='+map.idcard+'" >'+name+','+idcard+'</a>' +
                        '</h3>' +
                        '<div style="font-size: 12px;color: #797979">'+"专业："+majorShow+'</div>'+
                        '</div></div>';
                    table.appendChild(li);
                });
            }
    ...
    function addEnrollmentStudent() {
        window.location.href = "<%=request.getContextPath()%>/enrollmentApp/editEnrollmentStudentByApp";
    }
