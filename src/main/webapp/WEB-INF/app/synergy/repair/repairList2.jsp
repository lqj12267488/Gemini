...
<header class="mui-bar mui-bar-nav">
    ...
    <h1 id="title" class="mui-title">后勤维修</h1>
...
<script type="text/javascript">
    ...
    function pulldownRefresh() {
        ...
        setTimeout(function () {
            ...
            $.ajax({
                type: "POST",
                url: "<%=request.getContextPath()%>/repair/getRepairApp?flag=${flag}",
                data: {"page": page},
                async: false,
                dataType: 'json',
            ...
            function createEmlist(jsonStr) {
                ...
                $.each(emlistData, function (key, map) {
                    ...
                    var title = map.itemNameShow;
                    var li = document.createElement('li');
                    li.className = 'mui-table-view-cell';
                    li.innerHTML = '<div class="mui-table" open-type="common" ' +
                        'href="<%=request.getContextPath()%>/repair/repair?id=' + map.repairID + '&flag=${flag}">' +
                        '<div class="mui-table-cell mui-col-xs-10">' +
                        '<h3 class="mui-ellipsis">' +
                        title +
                        '</h3>' +
                        '<div style="font-size: 12px;color: #797979">' + map.requestFlag + '</div>' +
                        '</div></div>';
    ...
    function pullupRefresh() {
        setTimeout(function () {
            ...
            if (page == 1) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<%=request.getContextPath()%>/repair/getRepairApp",
                    data: {"page": page, flag: "${flag}"},
                    async: false,
                    dataType: 'json',
                    success: function (jsonData) {
                        createEmlist(jsonData);
