...
<header class="mui-bar mui-bar-nav">
    ...
    <h1 id="title" class="mui-title">请选择任务</h1>
...
<script type="text/javascript">
    ...
    function pulldownRefresh() {
        ...
        setTimeout(function() {
            ...
            $.ajax({
                type: "POST",
                url: "<%=request.getContextPath()%>/evaluationApp/result/getTaskNameDownRefresh",
                data:{"page":page,"evaluationType":eType},
                async: false,
                dataType: 'json',
...
//                    var date = new Date();
//                    var endTime1 = map.endTime.replace(/-/g, '/');
//                    var edate = new Date(endTime1);
                    var AinnerHTML ="";

                    if(map.validFlag == '1'){
                        AinnerHTML = '<a open-type="common" ' +
                                ' href="<%=request.getContextPath()%>/evaluationApp/result/listEmpsMenmbers?evaluationType='+eType+'&taskId='+map.taskId+'" >'
                                        +tasknameData+'</a>';
                    }else{
                        AinnerHTML = '<a open-type="common"' +
                                '  href="<%=request.getContextPath()%>/evaluationApp/result/overdueData?evaluationType='+eType+'&taskId='+map.taskId+'" style="color:#0062cc" >(已超期)'
                                             +tasknameData+'</a>';
                    }
                    li.innerHTML = '<div class="mui-table" >'+
                        '<div class="mui-table-cell mui-col-xs-10">'+
                        '<h3 class="mui-ellipsis">' +AinnerHTML +
                        '</h3>' +
...
/*
                    li.innerHTML = '<div class="mui-table" >'+
                        ...
                        '<a open-type="common"  href="/evaluationApp/result/listEmpsMenmbers?evaluationType='+eType+'&taskId='+map.taskId+'" >'
                        +tasknameData+'</a>' +
                        '</h3>' +
                        '<p class="mui-h6 mui-ellipsis">评教时间'+map.startTime +"至"+ map.endTime +'</p>'+
                        '</div></div>';
    ...
    function pullupRefresh() {
        setTimeout(function() {
            ...
            if(page == 1){
                ...
                $.ajax({
                    type: "POST",
                    url: "<%=request.getContextPath()%>/evaluationApp/result/getTaskNameUpRefresh",
                    data:{"page":page,"evaluationType":eType},
                    async: false,
                    dataType: 'json',
                    success: function(jsonData){
