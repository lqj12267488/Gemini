...
<script>
    ...
    $(document).ready(function () {
        ...
        arrayClassGrid = $("#arrayClassGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/core/parent/getArrayClassList',
            },
            "destroy": true,
            "columns": [
                ...
                {"width": "22%", "data": "remark", "title": "备注"},
                {
                    ...
                    "render": function (data, type, row) {
                        return "<a id='select' class='icon-th-list' title='查看学生课表'></a>";
                    }
                }
            ],
            'order' : [[1,'desc'],[2,'desc']],
        ...
        arrayClassGrid.on('click', 'tr a', function () {
            var data = arrayClassGrid.row($(this).parent()).data();
            var id = data.arrayClassId;
            if (this.id == "select") {
                $("#right").load("<%=request.getContextPath()%>/core/parent/lookStudentViewCourse?"
                    +"arrayclassId="+id+"&arrayClassName="+data.arrayClassName);
            }
        });
    })
    ...
    function search() {
        ...
        arrayClassGrid.ajax.url("<%=request.getContextPath()%>/core/parent/getArrayClassList" +
