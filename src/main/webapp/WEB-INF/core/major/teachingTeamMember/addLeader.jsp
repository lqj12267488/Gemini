    ...
    <div class="modal-content block-fill-white">
        ...
        <div class="modal-body clearfix">
            ...
            <div class="controls" id="style-4" style="overflow-y:auto;height:70% ">
                <input id="teamId" hidden value="${teamId}"><ul id="treeDemo" class="ztree"></ul>
            </div>
        </div>
        <div class="modal-footer">
            <button class="btn btn-success btn-clean" id="saveBtn" onclick="saveLeaderEmpDeptRelation()">保存</button>
...
<script>
    ...
    $(document).ready(function () {
        $.get("<%=request.getContextPath()%>/teachingTeam/getDeptAndPersonTree?teamId=" + $("#teamId").val()+"&memberType=1", function (data) {
            EmpDeptTree = $.fn.zTree.init($("#treeDemo"), setting, data);
            EmpDeptTree.expandAll(true);
        });
    });

    function saveLeaderEmpDeptRelation() {
        ...
        showSaveLoading();
        $.post("<%=request.getContextPath()%>/majorLeader/savePerRelation", {
            teamId: teamId,
            checkList: checkList,
            memberType:1
        }, function (msg) {
            ...
            if (msg.status == 1) {
                ...
                $("#dialog").modal('hide');
                $('#tableTeaching').DataTable().ajax.reload();
            }
        })
    }
</script>
