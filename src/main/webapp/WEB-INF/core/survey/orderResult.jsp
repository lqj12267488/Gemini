                    ...
                    <div class="form-row" id="syllabus" >
                        <div class="col-md-12">
                            <table width="100%" border="1" cellspacing="0">
                                <tr ><td colspan="2" style="text-align: center;font-size: 25px">${data.surveyTitle}</td></tr>
                                <c:forEach items="${questionList}" var="questionItem">
                                    <tr>
                                        <c:if test="${questionItem.questionType == '1'}">
                                            <td style="width: 30%;text-align: left">
                                                    ${questionItem.questionName}
                                            </td>
                                            <td style="width: 70%;text-align: left;">
                                                <c:forEach items="${questionItem.result}" var="resultItem">
                                                    ${resultItem}<br>
                                                </c:forEach>
                                            </td>
                                        </c:if>
                                        <c:if test="${questionItem.questionType == '2' || questionItem.questionType == '3'
                                            ||  questionItem.questionType == '4' || questionItem.questionType == '5'}">
                                            <td>
                                                <table style="width: 100%">
                                                    <tr><td style="border-top: 0px;text-align: left">${questionItem.questionName}</td></tr>
                                                </table>
                                            </td>
                                            <td>
                                                <table>
                                                    <c:forEach items="${questionItem.optionList}" var="oList">
                                                        <tr>
