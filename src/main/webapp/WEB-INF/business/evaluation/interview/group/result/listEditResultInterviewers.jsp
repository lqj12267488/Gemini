...
<div class="modal-dialog">
    <div class="modal-content block-fill-white">
        <div id="layout" class="loading"
             style="display:none;z-index:999;position:absolute;width: 100%;height: 100%;text-align: center">
            <br/><br/><br/>
            <img src="<%=request.getContextPath()%>/libs/img/app/loading.gif"
        ...
        <div class="modal-header">
            ...
            <table border="0" cellpadding="0" cellspacing="0">
                <tr>
                    <td>
                        <h4 class="modal-title">评教任务：</h4></td>
                    <td>
                        <h4 class="modal-title">${taskName}</h4></td>
                    <td>
                        <h4 class="modal-title">被评教人：</h4></td>
                    <td>
                        <h4 class="modal-title">${empName}</h4></td>
                </tr>
                <tr>
                    <td><h4 class="modal-title">姓名：${interviewerName}</h4></td>
                    <td><h4 class="modal-title">性别：${interviewerSex} </h4></td>
                    <td><h4 class="modal-title">民族：${interviewerNation}</h4></td>
                    <td><h4 class="modal-title">职称： ${interviewerPost}</h4></td>
                </tr>
                <tr>
                    <td><h4 class="modal-title">毕业院校：${interviewerGraduateSchool}</h4></td>
                    <td><h4 class="modal-title">专业： ${interviewerMajor}</h4></td>
                    <td><h4 class="modal-title">学历： ${interviewersEducation}</h4></td>
                    <td><h4 class="modal-title">毕业时间： ${interviewersTime} </h4></td>
                </tr>
                <tr>
                    <td><h4 class="modal-title">个人薪资要求：</h4></td>
                    <td><h4 class="modal-title">${interviewersSalary}</h4></td>
                    <td><h4 class="modal-title">本人求职岗位：</h4></td>
                    <td><h4 class="modal-title">${interviewersJob}</h4></td>
                </tr>
            </table>
        </div>
        <div class="modal-body clearfix">
            <div class="controls">
                            ...
                            <c:choose>
                                <c:when test="${indexListSe.leafFlag == 0 }">
                                    <div class="form-row">
                                        ...
                                        <div class="col-md-11"
                                             style="background:#13A4FB"><%--style="background: #7968EC"--%>
                                    ...
                                    <c:forEach items="${indexListSe.indexList}" var="indexListTre">
                                        ...
                                        <div class="form-row">
                                            ...
                                            <div class="col-md-10"
                                                 style="background:#69C5FC"><%--style="background:#DCA0DC"--%>
                ...
                <div class="form-row">
                    ...
                    <div class="col-md-10">
                        <textarea id="f_interviewEvaluate" type="text" name="remark" width="100%"></textarea>
                    </div>
                </div>
                <div class="form-row">
                    <div class="col-md-2 tar">
                    ...
                    <div class="col-md-10">
                        <select id="f_interviewDecision" name="remark" width="100%"/>
                    </div>
                </div>
                <div class="form-row tar">
                    <button type="button" id="saveBtn" class="btn btn-success btn-clean" onclick="save()">提交</button>
                    <button type="button" id="closeBtn" class="btn btn-default btn-clean" data-dismiss="modal">关闭
                    </button>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function () {
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=MSJD", function (data) {
            addOption(data, 'f_interviewDecision');
        });
    })
    function save() {
    ...
    function insertResult(returnValue) {
        ...
        $.post("<%=request.getContextPath()%>/evaluation/result/insertResultInterviewers",
            {
                taskId: $("#taskId").val(),
                ...
                interviewDecision:$("#f_interviewDecision option:selected").val(),
                interviewEvaluate:$("#f_interviewEvaluate").val()
            },
            function (msg) {
