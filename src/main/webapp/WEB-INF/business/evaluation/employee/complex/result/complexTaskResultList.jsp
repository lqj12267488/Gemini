    ...
    <div class="row">
        <div class="col-md-12">
            <div class="block">
                <div class="content">
                    <div class="header">
                        <span style="font-size: 15px;margin-left: 20px">${complexTaskName} > 查看评价结果</span>
                    </div>
                    <div class="form-row">
...
<script>
    ...
    $(document).ready(function () {
        var th1 = '<th rowspan="2">被评人</th><th rowspan="2">被评人部门</th>' +
            '<th rowspan="2">得分</th>';
        var th4 ='';// '<th rowspan="$ {maxLevel + 1}">操作</th>';
        <c:forEach items="${details}" var="index">
            th1 += '<th rowspan="1" colspan="2">' + '<a title="原评教名称：${index.taskName}">${index.taskShowName}</a>'+ '</th>';
        </c:forEach>
        $("#tr1").html(th1);
        /*"url": '/evaluation/getDetails?id=' + '$ {id}' + '&taskId=' + '$ {taskId}',*/
        taskDetailTable = $("#taskDetailTable").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/evaluation/complex/getComplexResultList?' +
    ...
    function back() {
        $("#right").load("<%=request.getContextPath()%>/evaluation/complex/getComplexResult");
    }
    ...
    function method1(tableid) {//整个表格拷贝到EXCEL中
        if(getExplorer()=='ie'){
            ...
            try {
                var fname = oXL.Application.GetSaveAsFilename("评教结果.xls", "Excel Spreadsheets (*.xls), *.xls");
            } catch (e) {
    ...
    var tableToExcel = (function() {
        ...
        return function(table, name) {
            ...
            var ctx = {worksheet: '${complexTaskName}评教结果' || 'Worksheet', table: table.innerHTML}
