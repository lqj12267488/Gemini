...
<script>
    ...
    $(document).ready(function () {
        ...
        ComplexTaskTable = $("#ComplexTaskTable").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/evaluation/complex/getComplexTask?evaluationType='+eType
            },
            "destroy": true,
            "columns": [
                ...
                {"width": "10%", "data": "startTime", "title": "操作时间"},
                {
                    ...
                    "render": function (date,type,row) {
                        var htm ='<a class="icon-edit" title="修改" id="editComplexTask" />&nbsp;&nbsp;&nbsp;' +
                            '<a class="icon-cog" title="子评教管理" id="editChildTask" />&nbsp;&nbsp;&nbsp;' +
                            '<a class="icon-trash" title="删除" id="del" />&nbsp;&nbsp;&nbsp;' ;
                        if(row.lengths != null && row.lengths != '' && row.testFlag == '0')
                            htm = htm + '<a class="icon-camera-retro" title="被评教人校验列表" id="checkEmpsList" />&nbsp;&nbsp;&nbsp;'
                                + '<a class="icon-check" title="校验" id="checkComplexDetail" />&nbsp;&nbsp;&nbsp;';
                        if(row.testFlag == '1')
                            htm = htm + '<a class="icon-camera-retro" title="被评教人校验列表" id="checkEmpsList" />&nbsp;&nbsp;&nbsp;'
                                + '<a class="icon-cogs" title="重新生成评教结果" id="insertResult" />&nbsp;&nbsp;&nbsp;' ;
                        return  htm;
                    }
                }
            ],
//            'order' : [[1,'desc'],[7,'asc']],
        ...
        ComplexTaskTable.on('click', 'tr a', function () {
            ...
            if(this.id == "editComplexTask"){// 查看
                $("#dialog").load( "<%=request.getContextPath()%>/evaluation/complex/editComplexTask?complexTaskId="+data.complexTaskId+"&testFlag="+data.testFlag);
                $("#dialog").modal("show");
            }else if(this.id == "editChildTask"){// 子评教管理
                $("#right").load("<%=request.getContextPath()%>/evaluation/complex/editChildTask?" +
                    "complexTaskId="+data.complexTaskId+
                    "&term="+data.term+"&complexTaskName="+data.complexTaskName+"&testFlag="+data.testFlag+"&evaluationType="+eType);
                ...
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/evaluation/complex/delComplexTask?complexTaskId="+data.complexTaskId, function (msg) {
                        if (msg.status == 1) {
                            swal({
                                title: msg.msg,
                                type: "success"
                ...
                /*if (confirm("请确认是否要删除本条申请?")) {
                    $.get("/evaluation/complex/delComplexTask?complexTaskId="+data.complexTaskId, function (msg) {
                        if (msg.status == 1) {
                            alert(msg.msg);
                            $('#ComplexTaskTable').DataTable().ajax.reload();
                        }
                ...
                $.get("<%=request.getContextPath()%>/evaluation/complex/checkChildTask?complexTaskId="+data.complexTaskId, function (msg) {
                    if (msg.status == 1) {
            ...
            }else if(this.id == "checkEmpsList"){// 被评教人校验列表
    ...
    function insertResult(complexTaskId) {
        $.get("<%=request.getContextPath()%>/evaluation/complex/insertResult?complexTaskId="+complexTaskId, function (msg) {
            swal({title: msg.msg, type: "success"});
            if (msg.status == 1) {
                $('#ComplexTaskTable').DataTable().ajax.reload();
            }
    ...
    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/evaluation/complex/addComplexTask?evaluationType="+eType);
        $("#dialog").modal("show");
    }
    ...
    function search() {
        ...
        ComplexTaskTable.ajax.url("<%=request.getContextPath()%>/evaluation/complex/getComplexTask?"+
