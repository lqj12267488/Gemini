...
<script>
    ...
    function search() {
        ...
        groupTable = $("#groupTable").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/evaluation/getGroupList',
                "data": {
                    groupName: groupName ,
            ...
            "columns": [
                ...
                {"width": "25%", "data": "groupType", "title": "评委组类型"},
                {"width": "25%", "data": "membersNum", "title": "评委数量"},
                {
                    ...
                    "render": function (data, type, row) {
                        return '<span class="icon-user" title="添加教师评委" ' +
                            ...
                            '<span class="icon-book" title="添加学生评委" ' +
                                    ' onclick="addMember(\'' +  row.groupId + '\',\'1\')"/>&ensp;&ensp;' +
                            '<span class="icon-tags" title="添加社会评委" ' +
                                    ' onclick="addMember(\'' +  row.groupId + '\',\'2\')"/>&ensp;&ensp;' +
                            '<span class="icon-group" title="添加领导评委" ' +
                                    ' onclick="addMember(\'' +  row.groupId + '\',\'3\')"/>&ensp;&ensp;' +
                            '<span class="icon-male" title="添加同行评委" ' +
                                    ' onclick="addMember(\'' +  row.groupId + '\',\'4\')"/>&ensp;&ensp;' +
    ...
    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/evaluation/addGroup");
        $("#dialog").modal("show");
    }
    ...
    function del(id, groupName) {
       ...
       /* if (confirm("确定要删除" + groupName + "?")) {
            $.post("/evaluation/deleteGroup", {
                id: id
            }, function (msg) {
                alert(msg.msg);
                //groupTable.ajax.reload();
    ...
    function edit(id) {
        $("#dialog").load("<%=request.getContextPath()%>/evaluation/toEditGroup?id=" + id);
        $("#dialog").modal("show");
    }
</script>