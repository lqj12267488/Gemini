...
<script>
    ...
    $(document).ready(function () {
        $.get("<%=request.getContextPath()%>/evaluation/getEmpsByGroup?id=" + "${id}" + "&memberType=" + '${memberType}', function (data) {
            groupTree = $.fn.zTree.init($("#groupTree"), setting, data.tree);
            groupTree.expandAll(false);
            var node = groupTree.getNodeByParam("id", "001");
            groupTree.expandNode(node, true, false, false);
            var selected = data.selected;
            for (var i = 0; i < selected.length; i++) {
                var node = groupTree.getNodeByParam("id", selected[i].personId+","+selected[i].deptId+","+selected[i].classId);
    ...
    function save() {
        ...
        $.post("<%=request.getContextPath()%>/evaluation/saveMember", {
            groupId: $("#groupId").val(),
            ids: id,
