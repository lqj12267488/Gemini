...
<script>
                ...
                {"width": "10%", "data": "schedule", "title": "进度"},
                {
                    ...
                    "render": function (data, type, row) {
                        return '<span class="icon-edit" title="查看" onclick=toEmps("' + row.taskId +
                            ...
                            '<span class="icon-search" title="查看图表" onclick=del("' + row.taskId +
                            '","' + row.taskName + '")/>';
                    },
                    "width": "8%"
                }
    ...
    function toEmps(id, taskName) {
        $("#right").load("<%=request.getContextPath()%>/evaluation/statistics/resultLeader", {
            id: id,
            taskName: taskName,
    ...
    function del(id, taskName) {
        $("#right").load("<%=request.getContextPath()%>/evaluation/leaderEcharts", {
            id: id,
            taskName: taskName,
            evaluationType:eType,
        });
    ...
    function search() {
        ...
        taskResultTable.ajax.url("<%=request.getContextPath()%>/evaluation/getMonitoerLeaderTask"
