...
<script>
    ...
    $(document).ready(function () {
        taskTable = $("#taskTable").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/evaluation/getMonitoerTeacherTaskBySelf?evaluationType='+eType,
            },
            "destroy": true,
            "columns": [
                ...
                {"width": "10%", "data": "schedule", "title": "进度"},
                {
                    ...
                    "render": function (data, type, row) {
                        return '<span class="icon-edit" title="详细" onclick=toEmps("' + row.taskId +
                            '","' + row.taskName + '")/>';
                    },
                    "width": "8%"
                }
    ...
    function toEmps(id, taskName) {
        $("#dialog").load("<%=request.getContextPath()%>/evaluation/loading");
        $("#dialog").modal("show");

        $("#right").load("<%=request.getContextPath()%>/evaluation/statistics/detailsByAllSelf", {
            id: id,
            taskName: taskName,
    ...
    function search() {
        ...
        taskTable.ajax.url("<%=request.getContextPath()%>/evaluation/getMonitoerTeacherTaskBySelf"
