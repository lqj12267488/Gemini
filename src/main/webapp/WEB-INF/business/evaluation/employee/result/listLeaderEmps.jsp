...
<script>
    ...
    $(document).ready(function () {
        ...
        empEvalTable = $("#empEvalTable").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/evaluation/result/getlisteLeaderMenmbers?evaluationFlag=0&evaluationType=' + eType
            },
            "destroy": true,
            "columns": [
...
//                        '<span class="icon-user" title="审核" onclick=evalutionEmp("' + row.taskId + '","'+ row.empPersonId+ '","'+ row.empDeptId+ '","'+ row.endTime + '","'+ row.planId + '","'+ row.taskName + '")/>';
    ...
    function search() {
        ...
        empEvalTable.ajax.url("<%=request.getContextPath()%>/evaluation/result/getlisteLeaderMenmbers?" +
            "empPersonId=" + empPersonId + "&" +
            "evaluationFlag=0" + "&" +
