    ...
    function addHoverDom(treeId, treeNode) {
        ...
        if (addBtu) addBtu.bind("click", function () {
            if (treeNode.level == 3) {
                ...
                $("#addIndex").show();
                $("#addIndex").load("<%=request.getContextPath()%>/xgEvaluation/toAddIndex", {planId: '${id}', pId: treeNode.id})
            }
            return false;
        });
    };
...
//        $("#editBtn_" + treeNode.tId).unbind().remove();
    ...
    function del(treeNode) {
        if (treeNode.id == 0 || treeNode.id == $("#planId").val() ) {
            ...
            swal({
                ...
                $.get("<%=request.getContextPath()%>/xgEvaluation/deleteIndex?id=" + treeNode.id, function (msg) {
                    swal({
                        title: msg.msg,
                        type: "success"
                    });
           ...
           /* if (confirm("确定要删除" + treeNode.name + "?")) {
                $.get("/xgEvaluation/deleteIndex?id=" + treeNode.id, function (msg) {
                    alert(msg.msg);
                    if (treeNode) {
                        zTree.removeNode(treeNode);
                    }
    ...
    function onClick(event, treeId, treeNode) {
        if (treeNode.level != 0) {
            $("#index").show();
            $("#addIndex").load("<%=request.getContextPath()%>/xgEvaluation/toEditIndex", {id: treeNode.id});
        } else {
            $("#index").hide();
        }
        return false;
    ...
    function back() {
        $("#right").load("<%=request.getContextPath()%>/xgEvaluation/plan");
    }
</script>
