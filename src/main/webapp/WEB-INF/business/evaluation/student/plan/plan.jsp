...
<script>
    ...
    $(document).ready(function () {
        planTable = $("#planTable").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/xgEvaluation/getPlanList?evaluationType=' + eType,
            },
            "destroy": true,
            "columns": [
                {"data": "planId", "visible": false},
                {"width": "30%", "data": "planName", "title": "考评方案名称"},
                {"width": "20%", "data": "score", "title": "总分数"},
                {"width": "30%", "data": "remark", "title": "备注"},
                {"width": "20%", "data": "testFlag", "title": "是否校验"},
                {
                    "width": "20%",
                    "title": "操作",
    ...
    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/xgEvaluation/addPlan");
        $("#dialog").modal("show");
    }

    function editPlan(id) {
        $("#dialog").load("<%=request.getContextPath()%>/xgEvaluation/toEditPlan", {
            id: id
        });
        $("#dialog").modal("show");
    }
    ...
    function editCopy(id) {
        swal({
            ...
            $.post("<%=request.getContextPath()%>/xgEvaluation/editCopyPlan", {
                id: id
            }, function (msg) {
                swal({
                    title: msg.msg,
    ...
    function del(id,planName) {
        swal({
            ...
            $.post("<%=request.getContextPath()%>/xgEvaluation/deletePlan", {
                id: id
            }, function (msg) {
                swal({
                    title: msg.msg,
        ...
        /*if (confirm("确定要删除考评方案 " + planName + " ?")) {
            $.post("/xgEvaluation/deletePlan", {
                id: id
            }, function (msg) {
                alert(msg.msg);
                planTable.ajax.reload();
    ...
    function searchclear() {
        $("#planNameSea").val("");
        planTable.ajax.url("<%=request.getContextPath()%>/xgEvaluation/getPlanList?evaluationType=" + eType).load();
    }

    function search() {
        var planName = $("#planNameSea").val();
        planTable.ajax.url("<%=request.getContextPath()%>/xgEvaluation/getPlanList?name=" + planName + "&evaluationType=" + eType).load();
    }

    function test(id) {
        $.post("<%=request.getContextPath()%>/xgEvaluation/checkPlan", {
            id: id
        }, function (msg) {
