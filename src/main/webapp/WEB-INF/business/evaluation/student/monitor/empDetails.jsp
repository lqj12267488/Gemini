...
<script>
    $(document).ready(function () {
        ...
        var th1 = '<th rowspan="${maxLevel + 1}">评委</th><th rowspan="${maxLevel + 1}">考评状态</th>' +
        ...
        taskTable = $("#taskTable").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/xgEvaluation/getDetails?id=' + '${id}' + '&taskId=' + '${taskId}',
            },
            "destroy": true,
            "columns": [
                {"data": "membersName"},
    ...
    function invalid(id, name) {
        $.get("<%=request.getContextPath()%>/xgEvaluation/checkInvalid?id=" + id, function (msg) {
            if (msg.status == 0) {
                $("#invalid").load("<%=request.getContextPath()%>/xgEvaluation/toInvalid", {
                    id: id,
                });
                $("#invalid").modal("show");
            } else {
    ...
    function back() {
        $("#right").load("<%=request.getContextPath()%>/xgEvaluation/toEmps", {
            id: '${taskId}',
            taskName: '${taskName}',
