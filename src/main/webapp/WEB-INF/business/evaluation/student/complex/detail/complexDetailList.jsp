...
<div class="content">
    <div class="col-md-12">
        <div class="block">
            <div class="header">
                <span style="font-size: 15px;margin-left: 20px">${complexTaskName} > 子考评管理</span>
            </div>
            <div class="content">
...
<script>
    ...
    $(document).ready(function () {
        ...
        ComplexDetailTable = $("#ComplexDetailTable").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/xgEvaluation/complex/getComplexDetail',
                "data": {
            ...
            "columns": [
                ...
                {"width": "30%", "data": "taskShowName", "title": "子考评名称"},
                {"width": "20%", "data": "taskName", "title": "包含考评"},
                {"width": "20%", "data": "weights", "title": "权重"},
                {"width": "20%", "data": "score", "title": "子考评原总分"},
                {
                    "width": "10%",
                    "title": "操作",
        ...
        ComplexDetailTable.on('click', 'tr a', function () {
            ...
            if (this.id == "edit") {
                $("#dialog").load("<%=request.getContextPath()%>/xgEvaluation/complex/viewComplexDetail?id=" + data.id);
                $("#dialog").modal("show");
            } else if (this.id == "del") {
                swal({
                    title: "您确定要删除本条信息？ ",
                    ...
                    $.get("<%=request.getContextPath()%>/xgEvaluation/complex/delChildTask?id=" + data.id, function (msg) {
                        if (msg.status == 1) {
                ...
                /*if (confirm("确定要删除此条子考评?")) {
                    $.get("/xgEvaluation/complex/delChildTask?id=" + data.id, function (msg) {
                        if (msg.status == 1) {
                            alert(msg.msg);
                            $('#ComplexDetailTable').DataTable().ajax.reload();
                        }
    ...
    function add() {
        $.post("<%=request.getContextPath()%>/xgEvaluation/complex/checkDetailWeights", {
            complexTaskId: $("#complexTaskId").val()
        }, function (msg) {
            if (msg.status == 1) {
                $("#dialog").load("<%=request.getContextPath()%>/xgEvaluation/complex/addComplexDetail" +
    ...
    function back() {
        $("#right").load("<%=request.getContextPath()%>/xgEvaluation/complex/getTaskList");
    }
</script>
