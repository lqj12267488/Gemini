...
<script>
    ...
    $(document).ready(function () {
        taskTable = $("#taskTable").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/xgEvaluation/getTasks?evaluationType=' + eType,
            },
            "destroy": true,
            "columns": [
                {"data": "taskId", "visible": false},
                {"width": "20%", "data": "taskName", "title": "考评任务名称"},
                {"width": "20%", "data": "planName", "title": "考评方案"},
                {"width": "8%", "data": "term", "title": "学期"},
                {"width": "10%", "data": "taskType", "title": "类型"},
                {"width": "7%", "data": "startTime", "title": "开始时间"},
                {"width": "7%", "data": "endTime", "title": "结束时间"},
                {"width": "9%", "data": "startFlagShow", "title": "启动<br>状态"},
                {
                    "width": "10%",
                    "title": "操作",
    ...
    function SetProportion(id,taskName) {
        $("#dialog").load("<%=request.getContextPath()%>/xgEvaluation/setProportion?id=" + id+"&taskName"+taskName);
        $("#dialog").modal("show");
    }

    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/xgEvaluation/toAddTask?evaluationType=" + eType);
        $("#dialog").modal("show");
    }

    function edit(id) {
        $("#dialog").load("<%=request.getContextPath()%>/xgEvaluation/toEditTask", {
            id: id ,
    ...
    function setStartFlag(id,startFlag) {
        ...
        swal({
            ...
            $.post("<%=request.getContextPath()%>/xgEvaluation/setStartFlagByTaskId", {
                taskId: id,
                startFlag: startFlag
            }, function (msg) {
                swal({
    ...
    function editCopy(id) {
        swal({
            ...
            $.post("<%=request.getContextPath()%>/xgEvaluation/editCopyTask", {
                id: id
            }, function (msg) {
                swal({
                    title: msg.msg,
    ...
    function del(id, taskName) {
        swal({
            ...
            $.post("<%=request.getContextPath()%>/xgEvaluation/delTask", {
                id: id
            }, function (msg) {
                swal({
                    title: msg.msg,
    ...
    function toSelectEmps(id, startFlag) {
        $("#dialog").load("<%=request.getContextPath()%>/xgEvaluation/toTaskSelectEmps", {
            id: id,
            startFlag: startFlag,
            evaluationType: eType,
        });
    ...
    function toSelectMembers(id, startFlag) {
        $("#dialog").load("<%=request.getContextPath()%>/xgEvaluation/toTaskSelectMembers", {
            id: id,
            startFlag: startFlag,
            evaluationType: eType,
        });
    ...
    function search() {
        ...
        taskTable.ajax.url("<%=request.getContextPath()%>/xgEvaluation/getTasks?name=" + taskName + "&evaluationType=" + eType).load();
    }
    ...
    function start(id, taskName) {
        swal({
            ...
            $.post("<%=request.getContextPath()%>/xgEvaluation/startTask", {
                id: id
            }, function (msg) {
                swal({
                    title: msg.msg,
