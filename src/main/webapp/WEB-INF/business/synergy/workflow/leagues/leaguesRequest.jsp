...
<script>
    ...
    $(document).ready(function () {
        leaguesTable = $("#leaguesTable").DataTable({
            "ajax": {
                "type": 'post',
                "url": '<%=request.getContextPath()%>/leagues/getLeaguesList',
            },
            "destroy": true,
            "columns": [
                ...
                {"width": "12%", "data": "remark", "title": "备注"},
                {
                    ...
                    "title": "操作",
                    "render": function () {
                        return "<a id='editLeagues' class='icon-edit' title='修改'></a>&nbsp;&nbsp;&nbsp;" +
                            "<a id='upload' class='icon-cloud-upload' title='上传附件'></a>&nbsp;&nbsp;&nbsp;"+
                            "<a id='deleteLeagues' class='icon-trash' title='删除'></a>&nbsp;&nbsp;&nbsp;" +
                            "<a id='relationLeagues' class='icon-exchange' title='关联事物'></a>&nbsp;&nbsp;&nbsp;" +
        ...
        leaguesTable.on('click', 'tr a', function () {
            ...
            if (this.id == "editLeagues") {
                $("#dialog").load("<%=request.getContextPath()%>/leagues/getLeagueById?id=" + id);
                $("#dialog").modal("show");
            }
            /*关联业务*/
            if (this.id == "relationLeagues") {
                $("#right").load("<%=request.getContextPath()%>/association/associtionList?id=" + id+"&type=0" +"&tableName="+$("#tableName").val()
                    +"&workflowCode="+$("#workflowCode").val()+"&backUrl="+"/leagues/request");
            }
            if (this.id == "deleteLeagues") {
                ...
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/leagues/deleteLeaguesById?id=" + id, function (msg) {
                        if (msg.status == 1) {
                            swal({
                                title: msg.msg,
                                type: "success"
            ...
            if (this.id == "upload"){
                $('#dialogFile').load('<%=request.getContextPath()%>/files/filesUpload?businessId=' + id + '&businessType=TEST&tableName=T_BG_LEAGUE_WF');
    ...
    function addLeagues() {
        $("#dialog").load("<%=request.getContextPath()%>/leagues/addLeague");
        $("#dialog").modal("show");
    }
    ...
    function search() {
        leaguesTable.ajax.url("<%=request.getContextPath()%>/leagues/getLeaguesList?requestDate="+$("#e_requestDate").val()).load();
    }
</script>
