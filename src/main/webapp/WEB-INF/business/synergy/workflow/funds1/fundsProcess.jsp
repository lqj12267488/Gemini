...
<script>
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            businessId: id,
                            tableName: "T_BG_FUNDS1_WF",
                            url: "/funds1/auditFundsById?id=" + id,
                            backUrl: "/funds1/process"
                        });
                    }
                })
            }
            if (this.id == "relationFunds") {
                $("#right").load("<%=request.getContextPath()%>/funds1/relationRequest?id=" + id+"&type=1");
            }
                ...
                $.post("<%=request.getContextPath()%>/getStartState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: id,
                            tableName: "T_BG_FUNDS1_WF",
                            url: "<%=request.getContextPath()%>/funds1/auditEdit?id=" + id,
                            backUrl: "<%=request.getContextPath()%>/funds1/process"
                        });
                    }
                })
            }
    ...
    function search() {
        ...
        roleTable = $('#funds1').DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/funds1/getFundsListProcess',
                "data": {
                    requestDate: requestDate,
                    requestDept: requestDept,
                    manager : manager
