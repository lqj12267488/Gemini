...
<script>
    ...
    $(document).ready(function () {
        roleTable = $("#relationFunds1Grid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/funds1/getRelationFundsList?id=${id}',
            },
            "destroy": true,
            "columns": [
    ...
    function addRelitionFunds() {
        $("#dialog").load("<%=request.getContextPath()%>/funds1/getWorkflowList?id=${id}");
        $("#dialog").modal("show");
    }
    ...
    function del(businessId) {
        ...
        swal({
            ...
            $.get("<%=request.getContextPath()%>/funds1/deleteRelationFundsById?id=${id}&businessId=" + businessId, function (msg) {
                if (msg.status == 1) {
                    swal({
                        title: msg.msg,
                        type: "success"
    ...
    function seeValues(businessId, url, tableName) {
        $("#dialogFile").load("<%=request.getContextPath()%>/funds1/getWotkflowLog?businessId=" + businessId + "&url=" + url + "&tableName=" + tableName+"&type=0");
        $("#dialogFile").modal("show");
    }

    function back() {
        if ('${type}' == "0") {
            $("#right").load("<%=request.getContextPath()%>/funds1/request");
        } else if ('${type}' == "1") {
            $("#right").load("<%=request.getContextPath()%>/funds1/process");
        } else {
            $("#right").load("<%=request.getContextPath()%>/funds1/complete");
        }
