...
<script>
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            businessId: deviceid,
                            tableName: "T_BG_CAMERA_WF",
                            url: "/camera/auditCamera?id=" + deviceid,
                            backUrl: "/camera/process"
                        });
                    }
                })
            }
                ...
                $.post("<%=request.getContextPath()%>/getStartState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: deviceid,
                            tableName: "T_BG_CAMERA_WF",
                            url: "<%=request.getContextPath()%>/camera/editCameraProcess?id=" + deviceid,
                            backUrl: "<%=request.getContextPath()%>/camera/process"
                        });
                    }
                })
            }
    ...
    function search() {
        ...
        hotelStayTable = $("#hotelStayGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/camera/getCameraListProcess',
                "data": {
                    requester: requester,
                    requestDate: date,
                    requestDept: dept
            ...
            "columns": [
                ...
                {"width": "8","title": "操作", "render": function () {return "<a id='transact' class='icon-file-text-alt' title='办理'></a>&nbsp;&nbsp;&nbsp;" +
