...
<script>
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            businessId: deviceid,
                            tableName: "T_BG_HOTELSTAY_WF",
                            url: "/hotelStay/auditHotelStay?id=" + deviceid,
                            backUrl: "/hotelStay/process"
                        });
                    }
                })
            }
                ...
                $.post("<%=request.getContextPath()%>/getStartState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: deviceid,
                            tableName: "T_BG_HOTELSTAY_WF",
                            url: "<%=request.getContextPath()%>/hotelStay/editHotelStayProcess?id=" + deviceid,
                            backUrl: "<%=request.getContextPath()%>/hotelStay/process"
                        });
                    }
                })
            }
    ...
    function search() {
        ...
        hotelStayTable = $("#hotelStayGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/hotelStay/getHotelStayListProcess',
                "data": {
                    requester: requester,
                    requestDate: date,
                    requestDept: dept
