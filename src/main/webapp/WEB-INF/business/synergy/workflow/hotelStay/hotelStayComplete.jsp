...
<script>
        ...
        hotelStayTable.on('click', 'tr a', function () {
            ...
            if (this.id == "uRole") {
                $("#right").load("<%=request.getContextPath()%>/getWotkflowLog", {
                    ...
                    url: "<%=request.getContextPath()%>/hotelStay/auditHotelStayById?id=" + assetsId,
                    backUrl: "/hotelStay/complete"
                });
            }
            //反馈
            if (this.id == "delRole") {
    ...
    function search() {
        ...
        hotelStayTable = $("#hotelStayGrid").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/hotelStay/getHotelStayListComplete',
                "data": {
                    requester: requester,
                    requestDate: requestDate,
                    requestDept: requestDept
   ...
   "columns": [
                ...
                {"width": "7%", "data": "reason", "title": "事由"},
                {"width": "9%", "data": "peopleNumber", "title": "住宿人数"},
                {"width": "9%", "data": "stayDays", "title": "住宿天数"},
                {"width": "7%", "data": "price", "title": "标准"},
