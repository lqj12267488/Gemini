...
<input id="tableName" hidden value="T_BG_FUNDS2_WF">
...
<script>
    ...
    $(document).ready(function () {
        ...
        roleTable.on('click', 'tr a', function () {
            ...
            var id = data.id;
            if (this.id == "uFunds") {
                $("#dialog").load("<%=request.getContextPath()%>/funds2/getFundsById?id=" + id);
                $("#dialog").modal("show");
            }
            if (this.id == "relationFunds") {
                $("#right").load("<%=request.getContextPath()%>/funds2/relationRequest?id=" + id+"&type=0");
            }
            if (this.id == "delFunds") {
                ...
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/funds2/deleteFundsById?id=" + id, function (msg) {
                        if (msg.status == 1) {
                            swal({
                                title: msg.msg,
                                type: "success"
    ...
    function addFunds() {
        $("#dialog").load("<%=request.getContextPath()%>/funds2/addFunds");
        $("#dialog").modal("show");
    }
    ...
    function search() {
        ...
        roleTable = $('#funds2').DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/funds2/search',
                "data": {
                    requestDate: requestDate,
                }
            },
