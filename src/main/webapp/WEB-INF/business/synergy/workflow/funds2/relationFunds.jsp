...
<script>
    ...
    function addRelitionFunds() {
        $("#dialog").load("<%=request.getContextPath()%>/funds2/getWorkflowList?id=${id}");
        $("#dialog").modal("show");
    }
    ...
    function back() {
        if ('${type}' == "0") {
            $("#right").load("<%=request.getContextPath()%>/funds2/request");
        } else if ('${type}' == "1") {
            $("#right").load("<%=request.getContextPath()%>/funds2/process");
        } else {
            $("#right").load("<%=request.getContextPath()%>/funds2/complete");
        }
