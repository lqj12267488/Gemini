...
<script>
    ...
    $(document).ready(function () {
        ...
        roleTable.on('click', 'tr a', function () {
            ...
            var id = data.id;
            if (this.id == "uFunds") {
                $("#dialog").load("<%=request.getContextPath()%>/funds/getFundsById?id=" + id);
                $("#dialog").modal("show");
            }
            if (this.id == "relationFunds") {
                $("#right").load("<%=request.getContextPath()%>/funds/relationRequest?id=" + id+"&type=0");
            }
            if (this.id == "delFunds") {
                ...
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/funds/deleteFundsById?id=" + id, function (msg) {
                        if (msg.status == 1) {
                            swal({
                                title: msg.msg,
                                type: "success"
    ...
    function addFunds() {
        $("#dialog").load("<%=request.getContextPath()%>/funds/addFunds");
        $("#dialog").modal("show");
    }
