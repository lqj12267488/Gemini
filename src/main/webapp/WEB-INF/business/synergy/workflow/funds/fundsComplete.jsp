...
<script>
        ...
        roleTable.on('click', 'tr a', function () {
            ...
            if (this.id == "handleFunds") {
                $("#right").load("<%=request.getContextPath()%>/getWotkflowLog", {
                    ...
                    url: "<%=request.getContextPath()%>/funds/auditFundsById?id=" + id,
                    backUrl: "/funds/complete"
                });
            }
            if (this.id == "relationFunds") {
                $("#right").load("<%=request.getContextPath()%>/funds/relationRequest?id=" + id +"&type=2");
            }
    ...
    function search() {
        ...
        roleTable = $('#funds').DataTable({
            ...
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/funds/getFundsListComplete',
                "data": {
                    requestDate: requestDate,
                    requestDept: requestDept,
                    manager : manager
