            ...
            <div class="block">
                ...
                <div class="block block-drop-shadow content">
                    <div class="form-row block" style="overflow-y:auto;">
                        <table id="funds" cellpadding="0" cellspacing="0"
...
<script>
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            businessId: id,
                            tableName: "T_BG_FUNDS_WF",
                            url: "/funds/auditFundsById?id=" + id,
                            backUrl: "/funds/process"
                        });
                    }
                })
            }
            if (this.id == "relationFunds") {
                $("#right").load("<%=request.getContextPath()%>/funds/relationRequest?id=" + id+"&type=1");
            }
                ...
                $.post("<%=request.getContextPath()%>/getStartState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: id,
                            tableName: "T_BG_FUNDS_WF",
                            url: "<%=request.getContextPath()%>/funds/auditEdit?id=" + id,
                            backUrl: "<%=request.getContextPath()%>/funds/process"
                        });
                    }
                })
            }
    ...
    function search() {
        ...
        roleTable = $('#funds').DataTable({
            ...
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/funds/getFundsListProcess',
                "data": {
                    requestDate: requestDate,
                    requestDept: requestDept,
                    manager : manager
