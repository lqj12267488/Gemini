            ...
            <div class="block">
                ...
                <div class="block block-drop-shadow content">
                    ...
                    <div class="form-row block" style="overflow-y:auto;">
                        <table id="mealCardHandleGrid" cellpadding="0" cellspacing="0"
                               width="100%"
...
<input id="tableName" hidden value="T_BG_MEALCARD_HANDLE_WF">
<input id="workflowCode" hidden value="T_BG_MEALCARD_HANDLE_WF01">
<input id="businessId" hidden>
<script>
    var mealCardHandleTable;

    $(document).ready(function () {
        search();
        mealCardHandleTable.on('click', 'tr a', function () {
            var data = mealCardHandleTable.row($(this).parent()).data();
            var id = data.id;
            if (this.id == "editmealCardHandle") {
                $("#dialog").load("<%=request.getContextPath()%>/mealCardHandle/getMealCardHandleById?id=" + id);
                $("#dialog").modal("show");
            }
            if (this.id == "delmealCardHandle") {
                swal({
                    title: "请确认是否要删除本条申请?",
                    type: "warning",
                    ...
                    $.get("<%=request.getContextPath()%>/mealCardHandle/deleteMealCardHandleById?id=" + id, function (msg) {
                        if (msg.status == 1) {
    ...
    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/mealCardHandle/editMealCardHandle");
        $("#dialog").modal("show");
    }
    ...
    function search() {
        ...
        mealCardHandleTable = $("#mealCardHandleGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/mealCardHandle/getMealCardHandleList',
                "data": {
                    mealCardTime: date
                }
            },
            ...
            "columns": [
                ...
                {"width": "20%", "data": "remark", "title": "备注"},
                {
                    ...
                    return  "<a id='editmealCardHandle' class='icon-edit' title='修改'></a>&nbsp;&nbsp;&nbsp;" +
                        "<a id='upload' class='icon-cloud-upload' title='上传附件'></a>&nbsp;&nbsp;&nbsp;"+
                        "<a id='delmealCardHandle' class='icon-trash' title='删除'></a>&nbsp;&nbsp;&nbsp;" +
