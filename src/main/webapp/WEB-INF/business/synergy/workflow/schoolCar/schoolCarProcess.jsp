...
<script>
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            businessId: deviceid,
                            tableName: "T_BG_SCHOOLCAR_WF",
                            url: "/schoolCar/auditSchoolCarById?id=" + deviceid,
                            backUrl: "/schoolCar/process"
                        });
                    }
                })
            }
                ...
                $.post("<%=request.getContextPath()%>/getStartState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: deviceid,
                            tableName: "T_BG_SCHOOLCAR_WF",
                            url: "<%=request.getContextPath()%>/schoolCar/editSchoolCarProcess?id=" + deviceid,
                            backUrl: "<%=request.getContextPath()%>/schoolCar/process"
                        });
                    }
                })
            }
    ...
    function search() {
        ...
        schoolCarTable = $("#schoolCarGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/schoolCar/getSchoolCarListProcess',
                "data": {
                    requester: requester,
                    requestDate: requestDate,
                    requestDept: requestDept
            ...
            "columns": [
                ...
                {"width": "4%","title": "操作","render": function () {return "<a id='transact' class='icon-file-text-alt' title='办理'></a>&nbsp;&nbsp;&nbsp;" +
