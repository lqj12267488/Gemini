...
<input id="tableName" hidden value="T_BG_SCHOOLCAR_WF">
...
<script>
    ...
    $(document).ready(function () {
        ...
        roleTable.on('click', 'tr a', function () {
            ...
            //修改
            if (this.id == "uRole") {
                $("#dialog").load("<%=request.getContextPath()%>/schoolCar/getSchoolCarById?id=" + id);
                $("#dialog").modal("show");
            }
            //删除
            if (this.id == "delRole") {
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/schoolCar/deleteSchoolCarById?id=" + id, function (msg) {
                        if (msg.status == 1) {
            ...
            if (this.id == "upload"){
                $('#dialogFile').load('<%=request.getContextPath()%>/files/filesUpload?businessId=' + id + '&businessType=TEST&tableName=T_BG_SCHOOLCAR_WF');
    ...
    /*添加函数*/
    function addAssets() {
        $("#dialog").load("<%=request.getContextPath()%>/schoolCar/addSchoolCar");
        $("#dialog").modal("show");
    }
    /*清空函数*/
    ...
    function searchHotelStay() {
        ...
        if (date != "")
            date = date;
        roleTable = $("#schoolCarGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/schoolCar/getSchoolCarList',
                "data": {
                    requestDate: date
                }
            },
            ...
            "columns": [
                ...
                {"width": "9%", "data": "carType", "title": "使用车型"},
                {"width": "9%", "data": "reason", "title": "申请事由"},
                {"width": "7%", "data": "peopleNum", "title": "人数"},
                {"width": "7%", "data": "remark", "title": "备注"},
                {"width": "5%","title": "操作","render": function () {return "<a id='uRole' class='icon-edit' title='修改'></a>&nbsp;&nbsp;&nbsp;" +
