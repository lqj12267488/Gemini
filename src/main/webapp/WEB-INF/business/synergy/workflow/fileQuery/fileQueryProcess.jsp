...
<script>
    ...
    $(document).ready(function () {
        ...
        roleTable.on('click', 'tr a', function () {
            ...
            //修改
            if (this.id == "eidtFileQueryProcess") {
                $.post("<%=request.getContextPath()%>/getStartState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: fileQueryProcessId,
                            tableName: "T_BG_FILEQUERY_WF",
                            url: "<%=request.getContextPath()%>/fileQuery/auditFileQueryEdit?id=" + fileQueryProcessId,
                            backUrl: "<%=request.getContextPath()%>/fileQuery/process"
                        });
                    }
                })
            }
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            businessId: fileQueryProcessId,
                            tableName: "T_BG_FILEQUERY_WF",
                            url: "/fileQuery/auditFileQueryById?id=" + fileQueryProcessId,
                            backUrl: "/fileQuery/process"
                        });
                    }
                })
            }
    ...
    function search() {
        ...
        roleTable = $("#fileQueryGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/fileQuery/getProcessList',
                "data": {
                    requester: jbr,
                    requestDate: date,
                    requestDept: dept
                ...
                {"width": "20%","data": "remark", "title": "备注"},
                {
                    ...
                    "render": function () {
                        return  "<a id='handleAudit' class='icon-file-text-alt' title='办理'></a>&nbsp;&nbsp;&nbsp;"+
                            "<a id='eidtFileQueryProcess' class='icon-edit' title='修改'></a>";
                    }
                },
            ],
            'order' : [1,'desc'],
