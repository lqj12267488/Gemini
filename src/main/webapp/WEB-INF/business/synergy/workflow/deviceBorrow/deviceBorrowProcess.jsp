...
<script>
    ...
    $(document).ready(function () {
        ...
        roleTable.on('click', 'tr a', function () {
            ...
            //修改
            if (this.id == "eidtDeviceBorrowProcess") {
                $.post("<%=request.getContextPath()%>/getStartState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: deviceBorrowProcessId,
                            tableName: "T_BG_DEVICEBORROW_WF",
                            url: "<%=request.getContextPath()%>/deviceBorrow/auditDeviceBorrowEdit?id=" + deviceBorrowProcessId,
                            backUrl: "<%=request.getContextPath()%>/deviceBorrow/process"
                        });
                    }
                })
            }
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            businessId: deviceBorrowProcessId,
                            tableName: "T_BG_DEVICEBORROW_WF",
                            url: "/deviceBorrow/auditDeviceBorrowById?id=" + deviceBorrowProcessId,
                            backUrl: "/deviceBorrow/process"
                        });
                    }
                })
            }
    ...
    function search() {
        ...
        roleTable = $("#deviceBorrowGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/deviceBorrow/getProcessList',
                "data": {
                    requester: jbr,
                    requestDate: date,
                    requestDept: dept
            ...
            "columns": [
                ...
                {"width": "12%", "title": "操作", "render": function () {return  "<a id='handleAudit' class='icon-file-text-alt' title='办理'></a>&nbsp;&nbsp;&nbsp;"+
                    "<a id='eidtDeviceBorrowProcess' class='icon-edit' title='修改'></a>";}},
