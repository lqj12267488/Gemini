...
<script>
        ...
        roleTable.on('click', 'tr a', function () {
            ...
            if (this.id == "handleBusinessCar") {
                $("#right").load("<%=request.getContextPath()%>/getWotkflowLog", {
                    ...
                    backUrl: "<%=request.getContextPath()%>/businessCar/car"
                });
            }
            //修改
            if (this.id == "uRole") {
                $("#dialog").load("<%=request.getContextPath()%>/businessCar/getBusinessCarByManager?id=" + id);
                $("#dialog").modal("show");
            }
        });
    })
    ...
    function search() {
        ...
        roleTable = $("#businessCar").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/businessCar/getBusinessCarListAll',
                "data": {
                ...
                {"width": "9", "data": "requesterConfirmFlag", "title": "确认状态"},
                {
                    ...
                    "render": function () {
                        return "<a id='handleBusinessCar' class='icon-search' title='查看流程轨迹'></a>&nbsp;&nbsp;&nbsp;"+
                            "<a id='uRole' class='icon-edit' title='登记'></a>";
                    }
                }
            ],
            'order' : [1,'desc'],
