...
<script>
    ...
    $(document).ready(function () {
        ...
        roleTable = $("#grantManagementGrid").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/grantmanagement/getProcessList',
            },
            "destroy": true,
            "columns": [
                {"data": "id", "visible": false},
                {"data": "createTime", "visible": false},
                {"width": "11%", "data": "requestDate", "title": "日期"},
                ...
                $.post("<%=request.getContextPath()%>/getStartState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: grantManagementProcessId,
                            tableName: "T_XG_GRANT_MANAGEMENT_WF",
                            url: "<%=request.getContextPath()%>/grantmanagement/auditGrantManagementEdit?id=" + grantManagementProcessId,
                            backUrl: "<%=request.getContextPath()%>/grantmanagement/process"
                        });
                    }
                })
            }
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            businessId: grantManagementProcessId,
                            tableName: "T_XG_GRANT_MANAGEMENT_WF",
                            url: "/grantmanagement/auditGrantManagementById?id=" + grantManagementProcessId, //2018年4月3日14:33:29修改url: "/studentProve/auditLeaveById?id=" + studentProveProcessId,
                            backUrl: "/grantmanagement/process"
                        });
                    }
                })
            }
