...
<input id="tableName" hidden value="T_XG_STUDENT_LEAVE_WF">
<input id="workflowCode" hidden>
<input id="businessId" hidden>
<input id="studentIdShow" hidden value="${studentLeave.studentId}">
<script>
    ...
    $(document).ready(function () {
            ...
            $.get("<%=request.getContextPath()%>/common/getSysDict?name=JTLX", function (data) {
                addOption(data, 'eleaveType', '${studentLeave.leaveType}');
            });
        ...
        leaveTable.on('click', 'tr a', function () {
            ...
            if (this.id == "editleave") {
                $("#dialog").load("<%=request.getContextPath()%>/studentLeave/getStudentLeaveById?id=" + id);
                $("#dialog").modal("show");
            }
            if (this.id == "delleave") {
                ...
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/studentLeave/deleteStudentLeaveById?id=" + id, function (msg) {
                        if (msg.status == 1) {
                            swal({
                                title: msg.msg,
                                type: "success"
            ...
            if (this.id == "upload"){
                $('#dialogFile').load('<%=request.getContextPath()%>/files/filesUpload?businessId=' + id + '&businessType=TEST&tableName=T_XG_STUDENT_LEAVE_WF');
    ...
    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/studentLeave/editStudentLeave");
        $("#dialog").modal("show");
    ...
    function search() {
        ...
        leaveTable = $("#leaveGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/studentLeave/getStudentLeaveList',
                "data": {
                    requestDate: date,
    ...
    function getAuditer(e) {
        if (e == 1) {
            $('#workflowCode').val("T_XG_STUDENT_LEAVE_WF03");
        }
        else if (e > 1 && e <= 3) {
            $('#workflowCode').val("T_XG_STUDENT_LEAVE_WF02");
        }
        else {
            $('#workflowCode').val("T_XG_STUDENT_LEAVE_WF01");
        }
