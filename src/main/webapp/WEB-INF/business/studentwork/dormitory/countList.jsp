...
<script>
    ...
    $(document).ready(function () {
        ...
        $.get("<%=request.getContextPath()%>/dorm/selectDormName", function (data) {
            $("#sdormName").autocomplete({
                source: data,
                select: function (event, ui) {
                    $("#sdormName").val(ui.item.label);
                    $("#sdormName").attr("keycode", ui.item.value);
                    return false;
                }
        ...
        dormTable = $("#dormGrid").DataTable({
            ...
            "columns": [
                ...
                {"width":"10%","data": "dormName", "title": "寝室名称"},
                {"width":"10%","data": "dormTypeShow", "title": "寝室类型"},
                {"width":"10%","data": "buildingId", "title": "楼宇"},
                {"width":"10%","data": "floor", "title": "所在楼层"},
                {"width":"10%","data": "peopleNumber", "title": "容纳人数"},
                {"width":"15%","data": "remark", "title": "备注"},
                {"width":"10%","title": "操作","render": function () {
                        return "<a id='searchLog' class='icon-comment-alt' title='查看操作日志'></a>";
                    }
                }
            ],
            'order' : [1,'desc'],
        ...
        dormTable.on('click', 'tr a', function () {
            ...
            if (this.id == "searchLog") {
                $("#dialog").load("<%=request.getContextPath()%>/dorm/searchLog?id=" + id);
                $("#dialog").modal("show");
            }
    ...
    function searchDorm() {
        var url = "<%=request.getContextPath()%>/dorm/getDormList?floor="+$("#floor option:selected").val()+"&dormName="+$("#sdormName").val();
        dormTable.ajax.url(url).load();
    }
</script>
