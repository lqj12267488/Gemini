        ...
        <div class="col-md-12">
            <div class="block">
                ...
                <div class="block block-drop-shadow content">
                    ...
                    <table id="standardGrid" cellpadding="0" cellspacing="0" width="100%"
...
<script>
    ...
    $(document).ready(function () {
        ...
        standardTable = $("#standardGrid").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/payment/newStudent/getStandardList?planId=${planId}',
            ...
            "columns": [
                {
                    "render": function (data, type, row) {
                        return "<input type='checkbox' name='checkbox' value='"+row.idcard+","+row.itemId+","+row.classId+"'/>";
                    }
                },
                {"data": "studentId", "visible": false},
                {"data": "idcard", "visible": false},
    ...
    function search() {
        ...
        if (majorDirection == undefined || majorDirection == null){
            majorDirection = ""
        }
        if (trainingLevel == undefined || trainingLevel == null){
            trainingLevel = ""
        }
        standardTable.ajax.url("<%=request.getContextPath()%>/payment/newStudent/getStandardList?departmentsId="+did+"&majorCode="+majorCode+"&majorDirection="+majorDirection+"&trainingLevel="+trainingLevel+"&classId="+cid+"&itemId="+item+"&name="+name+"&planId=${planId}").load();
        exportNewStudentPayment();
    }
    ...
    function searchclear() {
        ...
        $("#name").val("");
        $("#item").val("");
        standardTable.ajax.url("<%=request.getContextPath()%>/payment/newStudent/getStandardList?planId=${planId}").load();
    }
    function back() {
        $("#right").load("<%=request.getContextPath()%>/payment/newStudent");
    }
    function exportNewStudentPayment(){
        var did = $("#did option:selected").val();
        var mid = $("#mid option:selected").val();
        var cid = $("#cid option:selected").val();
        var item=$("#item").val();
        ...
        var majorDirection= mid.substring(7,9);
        var trainingLevel= mid.substring(10,mid.length);
        var hhh = "<%=request.getContextPath()%>/payment/exportNewStudentPayment?departmentsId="+did
    ...
    function showEmpDialog() {
        $("#dialog").load("<%=request.getContextPath()%>/payment/toImportPayment?planId=${planId}&flag=new");
        $("#dialog").modal("show");
    }
</script>
