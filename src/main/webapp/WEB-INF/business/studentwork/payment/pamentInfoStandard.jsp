...
<script>
    ...
    $(document).ready(function () {
        ...
        paymentInfoTable = $("#paymentInfoGrid").DataTable({
            "ajax": {
        ...
        exportInfoStudentPayment();
    });

    function addPaymentResult() {
        ...
        if (length >= 1) {
            ...
            $("#dialog").load("<%=request.getContextPath()%>/payment/info/addPaymentResult?ids=" + ids + "&check_value=" + check_value + "&planId=" + planId);
            $("#dialog").modal("show");
        } else {
            swal({
                title: "请选择要添加缴费结果的学生!",
    ...
    function search() {
       ...
       if (majorCode == null || majorCode == undefined){
        ...
        paymentInfoTable.ajax.url("<%=request.getContextPath()%>/payment/info/getStandardList?departmentsId=" + did + "&majorCode=" + majorCode + "&majorDirection=" + majorDirection + "&trainingLevel=" + trainingLevel + "&classId=" + cid + "&itemId=" + item + "&name=" + name + "&planId=${planId}").load();
        exportInfoStudentPayment();
    }
    ...
    function searchclear() {
        ...
        $("#name").val("");
        $("#item").val("");
        paymentInfoTable.ajax.url("<%=request.getContextPath()%>/payment/info/getStandardList?planId=${planId}").load();
    }
    function back() {
        $("#right").load("<%=request.getContextPath()%>/payment/info");
    }
    function exportInfoStudentPayment() {
        var did = $("#did option:selected").val();
        var mid = $("#mid option:selected").val();
        var cid = $("#cid option:selected").val();
        var item = $("#item").val();
        ...
        var majorDirection = mid.substring(7, 9);
        var trainingLevel = mid.substring(10, mid.length);
        var hhh = "<%=request.getContextPath()%>/payment/exportInfoStudentPayment?departmentsId=" + did
            + "&majorCode=" + majorCode + "&majorDirection=" + majorDirection + "&trainingLevel=" + trainingLevel
            + "&classId=" + cid + "&itemId=" + item + "&name=" + name + "&planId=${planId}&flag=1";
        $("#expdata").attr("href", hhh);

    }
    function showEmpDialog() {
        $("#dialog").load("<%=request.getContextPath()%>/payment/toImportPayment?planId=${planId}&flag=info");
        $("#dialog").modal("show");
    }
</script>
