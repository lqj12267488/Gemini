            ...
            <div class="block">
                ...
                <div class="block block-drop-shadow content">
                    ...
                    <div class="form-row block" style="overflow-y:auto;">
                        <table id="studentCadresGrid" cellpadding="0" cellspacing="0" width="100%"
...
<script>
    ...
    $(document).ready(function () {
        ...
        search();
        studentCadresTable.on('click', 'tr a', function () {
            var data = studentCadresTable.row($(this).parent()).data();
            var id = data.id;
            var studentId = data.studentId;
            if (this.id == "update") {
                $("#dialog").load("<%=request.getContextPath()%>/studentRewardPunish/studentCadres/edit?id=" + id);
                $("#dialog").modal("show");
            }
            if (this.id == "delete") {
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/studentRewardPunish/studentCadres/detele?id=" + id, function (msg) {
                        if (msg.status == 1) {
                            swal({
                                title: msg.msg,
                                type: "success"
    ...
    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/studentRewardPunish/studentCadres/edit" );
        $("#dialog").modal("show");
    }
    ...
    function search() {
        ...
        studentCadresTable = $("#studentCadresGrid").DataTable({
            "ajax": {
                "type":"post",
                "url": '<%=request.getContextPath()%>/studentRewardPunish/studentCadres/getList',
                "data":{
                    studentId: studentId,
                    title: title,
                    grantTime: grantTime
