...
<div class="modal-dialog">
        ...
        <div class="modal-body clearfix">
            ...
            <div class="controls" >
                ...
                <div class="form-row">
                    ...
                    <div class="col-md-4">
                        <input id="sex" type="text" value="${tuitionFree.sexShow}" key="${tuitionFree.sex}" readonly/>
                    </div>
                    <div class="col-md-2 tar">
                        身份证号
                    </div>
                    <div class="col-md-4">
                        <input id="idcard" type="text" value="${tuitionFree.idcard}" readonly/>
                    </div>
                </div>

                <div class="form-row">
                    <div class="col-md-2 tar">
                        <span class="iconBtx">*</span> 免金额(元)
                    </div>
                    <div class="col-md-4">
                        <input id="freeSum" type="text" value="${tuitionFree.freeSum}"/>
                    </div>
                    <div class="col-md-2 tar">
                        <span class="iconBtx">*</span> 减免年度
                    </div>
                    <div class="col-md-4">
                        <select id="freeYear" class="js-example-basic-single"></select>
                    </div>
                </div>
            </div>
        </div>
    ...
    <input id="id" value="${tuitionFree.id}" type="hidden" >
    <input id="dept" value="${tuitionFree.departmentsId}" type="hidden" >
    <input id="major" value="${tuitionFree.majorShow}" type="hidden" >
    <input id="class" value="${tuitionFree.classId}" type="hidden" >
    <input id="student" value="${tuitionFree.studentId}" type="hidden" >
...
//系统字典项
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=JMND", function (data) {
            addOption(data, 'freeYear','${tuitionFree.freeYear}');
        });
        $.get("<%=request.getContextPath()%>/common/getSelectDept?type=8", function (data) {
            addOption(data, 'departmentsId','${tuitionFree.departmentsId}');
        });
//通过系部id获取专业-方向-层次下拉列表
        ...
        $.get("<%=request.getContextPath()%>/common/getMajorShowByDeptId?deptId="+$("#dept").val(),function (data) {
            addOption(data, "majorShow",'${tuitionFree.majorShow}');
        })
//通过专业-方向-层次id获取班级下拉列表
        ...
        $.get("<%=request.getContextPath()%>/common/getClassByMajorShow?majorShow="+$("#major").val(), function (data) {
            addOption(data, "classId", '${tuitionFree.classId}');
        })
//通过班级id获取学生下拉列表
        ...
        $.get("<%=request.getContextPath()%>/common/getStudentByClassId?classId="+$("#class").val(), function (data) {
            addOption(data, "studentId",'${tuitionFree.studentId}');
        })
//通过学生获取性别和身份证号
...
//查看详情设置控件只读
    ...
    function save() {
        ...
        showSaveLoading();
        $.post("<%=request.getContextPath()%>/studentRewardPunish/tuitionFree/save", {
            id:$("#id").val(),
