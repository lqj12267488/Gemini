...
<div class="modal-dialog">
        ...
        <div class="modal-body clearfix">
            ...
            <div class="controls" >
                ...
                <div class="form-row">
                    ...
                    <div class="col-md-4">
                        <input id="sex" type="text" value="${grants.sexShow}" key="${grants.sex}" readonly/>
                    </div>
                    <div class="col-md-2 tar">
                        身份证号
                    </div>
                    <div class="col-md-4">
                        <input id="idcard" type="text" value="${grants.idcard}" readonly/>
                    </div>
                </div>

                <div class="form-row">
                    <div class="col-md-2 tar">
                        <span class="iconBtx">*</span>助学金类别
                    </div>
                    <div class="col-md-4">
                        <select id="grantType" class="js-example-basic-single"></select>
                    </div>
                    <div class="col-md-2 tar">
                        <span class="iconBtx">*</span> 助学金金额
                    </div>
                    <div class="col-md-4">
                        <input id="grantSum" type="text" value="${grants.grantSum}"/>
                    </div>
                </div>
                <div>
                    <div class="col-md-2 tar">
                    ...
                    <div class="col-md-10">
                        <select id="reportTerm" class="js-example-basic-single"></select>
                    </div>
                </div>
            </div>
        </div>
    ...
    <input id="id" value="${grants.id}" type="hidden" >
    <input id="dept" value="${grants.departmentsId}" type="hidden" >
    <input id="major" value="${grants.majorShow}" type="hidden" >
    <input id="class" value="${grants.classId}" type="hidden" >
    <input id="student" value="${grants.studentId}" type="hidden" >
...
//系统字典项
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=ZXJLB", function (data) {
            addOption(data, 'grantType','${grants.grantType}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=XQ", function (data) {
            addOption(data, 'reportTerm','${grants.reportTerm}');
        });
        $.get("<%=request.getContextPath()%>/common/getSelectDept?type=8", function (data) {
            addOption(data, 'departmentsId','${grants.departmentsId}');
        });
//通过系部id获取专业-方向-层次下拉列表
        ...
        $.get("<%=request.getContextPath()%>/common/getMajorShowByDeptId?deptId="+$("#dept").val(),function (data) {
            addOption(data, "majorShow",'${grants.majorShow}');
        })
//通过专业-方向-层次id获取班级下拉列表
        ...
        $.get("<%=request.getContextPath()%>/common/getClassByMajorShow?majorShow="+$("#major").val(), function (data) {
            addOption(data, "classId", '${grants.classId}');
        })
//通过班级id获取学生下拉列表
        ...
        $.get("<%=request.getContextPath()%>/common/getStudentByClassId?classId="+$("#class").val(), function (data) {
            addOption(data, "studentId",'${grants.studentId}');
        })
//通过学生获取性别和身份证号
...
//查看详情设置控件只读
    ...
    function save() {
        ...
        $.post("<%=request.getContextPath()%>/studentRewardPunish/grants/save", {
            id:$("#id").val(),
