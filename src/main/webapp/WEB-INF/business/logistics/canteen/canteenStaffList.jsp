...
<script>
    ...
    $(document).ready(function () {
        ...
        schoolBurseTable.on('click', 'tr a', function () {
            ...
            var id = data.id;
            if (this.id == "update") {
                $("#dialog").load("<%=request.getContextPath()%>/canteen/canteenStaffEdit?id=" + id);
                $("#dialog").modal("show");
            }
            if (this.id == "delete") {
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/canteen/canteenStaffDelete?id=" + id, function (msg) {
                        if (msg.status == 1) {
                            swal({
                                title: msg.msg,
                                type: "success"
    ...
    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/canteen/canteenStaffEdit" );
        $("#dialog").modal("show");
    }
    ...
    function search() {
        ...
        schoolBurseTable = $("#schoolBurseGrid").DataTable({
            "ajax": {
                "type":"post",
                "url": '<%=request.getContextPath()%>/canteen/canteenStaffList',
                "data":{
                    staffName: staffName
                }
            },
