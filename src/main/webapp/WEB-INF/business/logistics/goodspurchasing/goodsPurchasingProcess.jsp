...
<script>
    ...
    $(document).ready(function () {
        ...
        roleTable.on('click', 'tr a', function () {
            ...
            var goodsPurchasingProcessId = data.id;
            //修改
            if (this.id == "uGoodsPurchasingProcess") {
                $.post("<%=request.getContextPath()%>/getStartState", {
                    tableName: "T_ZW_GOODSPURCHASING_WF",
                    businessId: goodsPurchasingProcessId
                }, function (msg) {
                    if (msg.status == 1) {
                        swal({
                            title: msg.msg,
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: goodsPurchasingProcessId,
                            tableName: "T_ZW_GOODSPURCHASING_WF",
                            url: "<%=request.getContextPath()%>/goodsPurchasing/auditGoodsPurchasingEdit?id=" + goodsPurchasingProcessId,
                            backUrl: "<%=request.getContextPath()%>/goodsPurchasing/process"
                        });
                    }
                })
            }
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            businessId: goodsPurchasingProcessId,
                            tableName: "T_ZW_GOODSPURCHASING_WF",
                            url: "/goodsPurchasing/auditGoodsPurchasingById?id=" + goodsPurchasingProcessId,
                            backUrl: "/goodsPurchasing/process"
                        });
                    }
                })
            }
    ...
    function search(){
        ...
        roleTable = $('#goodsPurchasingGrid').DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/goodsPurchasing/getGoodsPurchasingListProcess',
                "data": {
                    requestDept: requestDept,
                    requester: requester,
                    requestDate: date
                ...
                },
                {
                    ...
                    "render": function () {
                        return "<a id='handleAudit' class='icon-file-text-alt' title='办理'></a>&nbsp;&nbsp;&nbsp;"  +
                                "<a id='uGoodsPurchasingProcess' class='icon-edit' title='修改'></a>";
                    }
                }
            ],
            'order' : [1,'desc'],
