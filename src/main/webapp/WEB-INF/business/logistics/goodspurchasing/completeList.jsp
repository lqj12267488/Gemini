...
<div class="modal-dialog" style="width: 1200px">
    <div class="modal-content block-fill-white">
        ...
        <div class="modal-footer">
            <button type="button" id="saveBtn" class="btn btn-default btn-clean" onclick="addRelation()" data-dismiss="modal">保存
            </button>
...
<script>
    ...
    $(document).ready(function () {
        ...
        workflow = $("#workflowGrid").DataTable({
            ...
            "columns": [
                ...
                {"width": "40%", "data": "text"},
                // {"width": "28%", "data": "reason"},
                {"width": "20%", "data": "createTime"},
                {
                    "width": "12%",
                    "title": "操作",
                    "render": function (data, type, row) {
    ...
    function search() {
        ...
        workflow.ajax.url("<%=request.getContextPath()%>/funds2/workflowList?id="+$("#id").val()+"&workflowId="+workflowId).load();
    }
    function seeValues(businessId,url,tableName) {
        $("#dialogFile").css("display","block");
        $("#dialogFile").load("<%=request.getContextPath()%>/funds2/getWotkflowLog?businessId="+businessId+"&url="+url+"&tableName="+tableName+"&type=1");
    }
    function checkAll() {
    ...
    function addRelation() {
        ...
        if (length >= 1) {
            ...
            $.post("<%=request.getContextPath()%>/funds2/saveRelationFunds", {
                ids: ids
            }, function (msg) {
                hideSaveLoading();
                swal({
