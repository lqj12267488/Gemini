...
<script>
    ...
    $(document).ready(function () {
        ...
        schoolBurseTable.on('click', 'tr a', function () {
            ...
            var id = data.id;
            if (this.id == "update") {
                $("#dialog").load("<%=request.getContextPath()%>/Supermarket/getSupermarketById?id=" + id);
                $("#dialog").modal("show");
            }
            if (this.id == "delete") {
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/Supermarket/checkSupermarketStaffById?id="+id, function (data) {
                        if(data.status ==1){
                            swal({
                                title: data.msg,
                                type: "error"
                            ...
                            $.get("<%=request.getContextPath()%>/Supermarket/SupermarketDelete?id=" + id, function (msg) {
                                if (msg.status == 1) {
                                    swal({
                                        title: msg.msg,
                                        type: "success"
    ...
    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/Supermarket/SupermarketEdit" );
        $("#dialog").modal("show");
    }
    ...
    function search() {
        ...
        schoolBurseTable = $("#schoolBurseGrid").DataTable({
            "ajax": {
                "type":"post",
                "url": '<%=request.getContextPath()%>/Supermarket/SupermarketList',
                "data":{
            ...
            "columns": [
                ...
                {"width":"10%","data": "name", "title": "超市名称"},
                {"width":"10%","data": "address", "title": "位置"},
                {"width":"10%","data": "personName", "title": "负责人"},
                {"width":"10%","data": "personidCard", "title": "身份证号"},
                {"width":"10%","data": "personTel", "title": "联系方式"},
