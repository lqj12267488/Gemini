...
<script>
        ...
        schoolBurseTable.on('click', 'tr a', function () {
            ...
            if (this.id == "delete") {
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/dormmanage/checktsudentDelete?studentDormId=" + studentDormId, function (msg) {
                        if (msg.status == 1) {
                            swal({
                                title: msg.msg,
                                type: "error"
    ...
    function search() {
        ...
        schoolBurseTable = $("#schoolBurseGrid").DataTable({
            "ajax": {
                "type":"post",
                "url": '<%=request.getContextPath()%>/dormmanage/studentDormList',
                "data":{
