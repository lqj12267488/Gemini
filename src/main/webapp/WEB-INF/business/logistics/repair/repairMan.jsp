...
<script>
    ...
    $(document).ready(function () {
        repairManGrid = $("#repairManGrid").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/repair/searchMan',
            },
            "destroy": true,
            "columns": [
                {"data": "id", "visible": false},
                {"data": "createTime", "visible": false},
                {"width":"34%","data": "personName", "title": "维修员姓名"},
                {"width":"33%","data": "deptName", "title": "维修员部门"},
                {
                    "width":"33%",
                    "title": "操作",
                    "render": function () {
                ...
                $.get("<%=request.getContextPath()%>/repair/getRepairByRepairMan?id=" + id, function (boolean) {
                    if (boolean == true) {
                        ...
                        swal({
                            ...
                            $.post("<%=request.getContextPath()%>/repair/delRepairMan", {
                                id: id
                            }, function (msg) {
                                swal({
                                    title: msg.msg,
    ...
    function addRepairMan() {
        $("#dialog").load("<%=request.getContextPath()%>/repair/addRepairMan");
        $("#dialog").modal("show");
    }
