...
<script>
    ...
    function dataTreeOnClick(event, treeId, treeNode) {
        if (treeNode.id != 0) {
            $("#addDept").show();
            $("#addDept").load("<%=request.getContextPath()%>/dataType/editDataType", {
                id: treeNode.id,
                name:treeNode.name
            });
            return false;
    ...
    function del(treeNode) {
        if (treeNode.id == 0) {
            ...
            swal({
                ...
                $.get("<%=request.getContextPath()%>/dataType/deleteDataType?id=" + treeNode.id, function (msg) {
                    swal({
                        title: msg.msg,
                        type: msg.result
                    });
    ...
    function addHoverDom(treeId, treeNode) {
        ...
        if (addBtu) addBtu.bind("click", function () {
            ...
            if(treeNode.id.length<7) {
                $("#addDept").show();
                $("#addDept").load("<%=request.getContextPath()%>/dataType/addDataType", {
                    pId: treeNode.id,
    ...
    $(document).ready(function () {
        $.get("<%=request.getContextPath()%>/dataType/getDataTypeTree", function (data) {
