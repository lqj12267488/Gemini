...
<script>
    ...
    $(document).ready(function () {
        ...
        table = $("#table").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/suggestionBox/student/getSuggestionList'
            },
            "order": [3, 'DESC'],
            "destroy": true,
            "columns": [
    ...
    <!-- 新增意见 -->
    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/suggestionBox/student/addSuggestion");
        $("#dialog").modal("show");
    }
    <!-- 查询 -->
    function search() {
        ...
        var url = '<%=request.getContextPath()%>/suggestionBox/student/getSuggestionList?title=' + searchTitle + '&replyFlag=' + searchReplyFlag;
    ...
    <!-- 意见详情 -->
    function detail(id) {
        $("#dialog").load("<%=request.getContextPath()%>/suggestionBox/student/suggestionDetail?id=" + id);
        $("#dialog").modal("show");
    ...
    function del(id) {
        ...
        swal({
            ...
            $.post("<%=request.getContextPath()%>/suggestionBox/student/suggestionDel",{
                id:id
            }, function (msg) {
                   if (msg.status == 1){
                       swal({
    ...
    function replyList(id) {
        $("#right").load("<%=request.getContextPath()%>/suggestionBox/student/getReplyListPage?id=" +id);
    }
</script>