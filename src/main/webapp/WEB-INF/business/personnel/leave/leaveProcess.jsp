...
<script>
                ...
                $.post("<%=request.getContextPath()%>/getStartState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: leaveProcessId,
                            tableName: "T_RS_LEAVE_WF",
                            url: "<%=request.getContextPath()%>/leave/auditLeaveEdit?id=" + leaveProcessId,
                            backUrl: "<%=request.getContextPath()%>/leave/process"
                        });
                    }
                })
            }
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            businessId: leaveProcessId,
                            tableName: "T_RS_LEAVE_WF",
                            url: "/leave/auditLeaveById?id=" + leaveProcessId,
                            backUrl: "/leave/process"
                        });
                    }
                })
            }
    ...
    function search() {
        ...
        roleTable = $("#leaveGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/leave/getProcessList',
                "data": {
                    requester: jbr,
                    requestDate: date,
                    requestDept: dept
            ...
            "columns": [
                ...
                {"width": "11%", "title": "操作", "render": function () {return "<a id='handleAudit' class='icon-file-text-alt' title='办理'></a>&nbsp;&nbsp;&nbsp;"+
