            ...
            <div class="block">
                ...
                <div class="block block-drop-shadow content">
                    ...
                    <div class="form-row block" style="overflow-y:auto;">
                        <table id="groupTrainingGrid" cellpadding="0" cellspacing="0" width="100%"
...
<input id="workflowCode" hidden value="T_RS_TRAINING_WF03">
...
<script>
    ...
    $(document).ready(function () {
        ...
        table = $("#groupTrainingGrid").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/training/group/getTrainingList',
            },
            "destroy": true,
            "columns": [
                ...
                {"width": "10%", "title": "操作", "render": function () {return "<a id='editTraining' class='icon-edit' title='修改'></a>&nbsp;&nbsp;&nbsp;" +
        ...
        table.on('click', 'tr a', function () {
            ...
            if (this.id == "editTraining") {
                $("#dialog").load("<%=request.getContextPath()%>/training/group/getTrainingById?id=" + id);
                $("#dialog").modal("show");
            }
            if (this.id == "delTraining") {
                ...
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/training/group/deleteTrainingById?id=" + id, function (msg) {
                        swal({
                            title: msg.msg,
                            type: "success"
                        });
    ...
    function addTraining() {
        $("#dialog").load("<%=request.getContextPath()%>/training/group/editTraining");
        $("#dialog").modal("show");
    }
    function search() {
        var date = $("#date").val();
        ...
        if (type == "" && date == "") {
            ...
            table.ajax.url("<%=request.getContextPath()%>/training/group/getTrainingList?trainingType="+type+"&requestDate="+date).load();
        }
    }
    ...
    function searchclear() {
        $("#date").val("");
        $("#type").val("");
        table.ajax.url("<%=request.getContextPath()%>/training/group/getTrainingList").load();
    }
