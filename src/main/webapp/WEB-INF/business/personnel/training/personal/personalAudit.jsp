...
<script>
    ...
    $(document).ready(function () {
        ...
        auditTable = $("#auditGrid").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/training/personal/getAuditList',
            },
            "destroy": true,
            "columns": [
                ...
                $.post("<%=request.getContextPath()%>/getStartState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: id,
                            tableName: "T_RS_TRAINING_WF",
                            url: "<%=request.getContextPath()%>/training/personal/auditPersonalEdit?id=" + id,
                            backUrl: "<%=request.getContextPath()%>/training/personal/audit"
                        });
                    }
                })
            }
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            ...
                            url: "/training/personal/auditPersonalById?id=" + id,
                            backUrl: "/training/personal/audit"
                        });
                    }
                })
            }
    ...
    function searchclear() {
        $("#dept").val("");
        $("#date").val("");
        auditTable.ajax.url("<%=request.getContextPath()%>/training/personal/getAuditList").load();
    }
    ...
    function search() {
        ...
        if (type == "" && date== "" ) {
            ...
            auditTable.ajax.url("<%=request.getContextPath()%>/training/personal/getAuditList?trainingType="+type+"&trainingDate="+date).load();
        }
