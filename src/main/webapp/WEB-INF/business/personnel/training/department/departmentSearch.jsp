...
<script>
    ...
    $(document).ready(function () {
        ...
        searchTable = $("#searchGrid").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/training/department/getSearchList',
            },
            "destroy": true,
            "columns": [
        ...
        searchTable.on('click', 'tr a', function () {
            ...
            if (this.id == "handleFlowTtrack") {
                $("#right").load("<%=request.getContextPath()%>/getWotkflowLog", {
                    ...
                    url: "<%=request.getContextPath()%>/training/department/auditDepartmentById?id=" + id,
                    backUrl: "/training/department/search"
                });
            }
            //反馈
                    ...
                    $.post("<%=request.getContextPath()%>/getBusinessStatus", {
                        ...
                        if (msg.status == 1) {
                            ...
                            $("#right").load("<%=request.getContextPath()%>/getFeedback", {
                                businessId: id,
                                tableName: "T_RS_TRAINING_WF",
                                backUrl: "<%=request.getContextPath()%>/training/department/search"
                            });
                        }
                    })
    ...
    function searchclear() {
        ...
        $("#date").val("");
        searchTable.ajax.url("<%=request.getContextPath()%>/training/department/getSearchList").load();
    }
    ...
    function search() {
        ...
        if (type == "" && date == "" ) {
            ...
            searchTable.ajax.url("<%=request.getContextPath()%>/training/department/getSearchList?trainingType="+type+"&trainingDate="+date).load();
        }
    }
</script>
