...
<script>
    ...
    $(document).ready(function () {
        ...
        auditTable = $("#auditGrid").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/training/department/getAuditList',
            },
            "destroy": true,
            "columns": [
                ...
                $.post("<%=request.getContextPath()%>/getStartState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toEditBusiness", {
                            businessId: id,
                            tableName: "T_RS_TRAINING_WF",
                            url: "<%=request.getContextPath()%>/training/department/auditDepartmentEdit?id=" + id,
                            backUrl: "<%=request.getContextPath()%>/training/department/audit"
                        });
                    }
                })
            }
                ...
                $.post("<%=request.getContextPath()%>/getRejectState", {
                    ...
                    if (msg.status == 1) {
                        ...
                        $("#right").load("<%=request.getContextPath()%>/toAudit", {
                            ...
                            url: "/training/department/auditDepartmentById?id=" + id,
                            backUrl: "/training/department/audit"
                        });
                    }
                })
            }
    ...
    function searchclear() {
        $("#dept").val("");
        $("#date").val("");
        auditTable.ajax.url("<%=request.getContextPath()%>/training/department/getAuditList").load();
    }
    ...
    function search() {
        ...
        if (type == "" && date== "" ) {
            ...
            auditTable.ajax.url("<%=request.getContextPath()%>/training/department/getAuditList?trainingType="+type+"&trainingDate="+date).load();
        }
