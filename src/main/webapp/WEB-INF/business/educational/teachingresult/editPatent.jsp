...
<div class="modal-dialog">
    <div class="modal-content block-fill-white">
        <div class="modal-header">
            ...
            <input id="w_id" hidden value="${patent.id}">
        ...
        <div class="modal-body clearfix">
            ...
            <div class="controls" >
                ...
                <div class="form-row">
                    ...
                    <div class="col-md-10">
                        <input id="w_personId" type="text"
                               value="${patent.personId}">
                    </div>
                </div>

                <div class="form-row">
                    <div class="col-md-2 tar">
                        <span class="iconBtx">*</span>证书名称：
                    </div>
                    <div class="col-md-10">
                        <input id="w_patentName" type="text" value="${patent.patentName}"
                ...
                <div class="form-row">
                    <div class="col-md-2 tar">
                        <span class="iconBtx">*</span>证书号：
                    </div>
                    <div class="col-md-10">
                        <input id="w_patentNum" type="text" maxlength="10" placeholder="最多输入10个字" value="${patent.patentNum}">
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="col-md-10">
                        <input id="w_noticeDate" type="date" value="${patent.noticeDate}">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="col-md-2 tar">
                        <span class="iconBtx">*</span> 专利类型：
                    </div>
                    <div class="col-md-10">
                        <select id="w_patentType" class="js-example-basic-single" ></select>
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="col-md-10">
                        <textarea id="w_remark" type="text"
                                  ...
                                  value="${patent.remark}">${patent.remark}</textarea>
                    </div>
                </div>
        ...
        <div class="modal-footer">
            <button id="saveBtn" type="button" class="btn btn-success btn-clean" onclick="savePatent()">保存</button>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=SDBXXM", function (data) {
            addOption(data, 'w_typicalFlag','${patent.typicalFlag}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=BRJSYY", function (data) {
            addOption(data, 'w_role','${patent.role}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=ZLZZLX", function (data) {
            addOption(data, 'w_patentType','${patent.patentType}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=XKLY", function (data) {
            addOption(data, 'w_subject','${patent.subject}');
        });
        $.post("<%=request.getContextPath()%>/files/getFilesByBusinessId", {
            businessId: '${patent.id}',
        }, function (data) {
        ...
        var ppid = $("#w_id").val();
        if(ppid!=""){
            $("#w_personId").val("${patent.personId}");
            $("#w_personId").attr("keycode", "${patent.deptId}");
        }
        if($("#flag").val()=='on'){
            $("#saveBtn").hide();
    ...
    function savePatent() {
        ...
        if($("#w_patentName").val()=="" || $("#w_patentName").val() =="0" || $("#w_patentName").val() == undefined){
            swal({
                title: "请填写专利(著作)名称！",
                type: "info"
            });
        ...
        if($("#w_patentNum").val()=="" || $("#w_patentNum").val() =="0" || $("#w_patentNum").val() == undefined){
            swal({
                title: "请填写专利(著作)号！",
                type: "info"
            });
        ...
        if($("#w_noticeDate").val()=="" || $("#w_noticeDate").val() =="0" || $("#w_noticeDate").val() == undefined){
            swal({
                title: "请选择批准日期！",
                type: "info"
            });
        ...
        if($("#w_patentType").val()=="" || $("#w_patentType").val() == undefined){
            swal({
                title: "请选择专利(著作)类型！",
                type: "info"
            });
        ...
        showSaveLoading();
        $.post("<%=request.getContextPath()%>/teacherResult/saveTeachingResult/patent", {
            id: pid,
            personId: headTList[1],
            deptId: headTList[0],
            patentName: $("#w_patentName").val(),
            patentNum: $("#w_patentNum").val(),
            typicalFlag: $("#w_typicalFlag").val(),
            role: $("#w_role").val(),
            patentType: $("#w_patentType").val(),
            subject: $("#w_subject").val(),
            noticeDate: $("#w_noticeDate").val(),
            remark: $("#w_remark").val()
        }, function (msg) {
            hideSaveLoading();
            if (msg.status == 1) {
