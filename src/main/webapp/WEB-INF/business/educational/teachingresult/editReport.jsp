...
<div class="modal-dialog">
    <div class="modal-content block-fill-white">
        <div class="modal-header">
            ...
            <input id="w_id" hidden value="${report.id}">
        ...
        <div class="modal-body clearfix">
            ...
            <div class="controls" >
                ...
                <div class="form-row">
                    ...
                    <div class="col-md-10">
                        <input id="w_personId" type="text"
                               value="${report.personId}">
                    </div>
                </div>

                <div class="form-row">
                    <div class="col-md-2 tar">
                        <span class="iconBtx">*</span>报告名称：
                    </div>
                    <div class="col-md-10">
                        <input id="w_reportName" type="text" value="${report.reportName}"
                ...
                <div class="form-row">
                    <div class="col-md-2 tar">
                        <span class="iconBtx">*</span>报告时间：
                    </div>
                    <div class="col-md-10">
                        <input id="w_reportDate" type="date" value="${report.reportDate}">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="col-md-2 tar">
                        <span class="iconBtx">*</span>委托方：
                    </div>
                    <div class="col-md-10">
                        <input id="w_consigner" type="text" value="${report.consigner}" maxlength="10" placeholder="最多输入10个字">
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="col-md-10">
                        <textarea id="w_remark" type="text"
                                  ...
                                  value="${report.remark}">${report.remark}</textarea>
                    </div>
                </div>
        ...
        <div class="modal-footer">
            <button id="saveBtn" type="button" class="btn btn-success btn-clean" onclick="saveReport()">保存</button>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=SDBXXM", function (data) {
            addOption(data, 'w_typicalFlag','${report.typicalFlag}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=BRJSYY", function (data) {
            addOption(data, 'w_role','${report.role}');
        });
        ...
        var ppid = $("#w_id").val();
        if(ppid!=""){
            $("#w_personId").val("${report.personId}");
            $("#w_personId").attr("keycode", "${report.deptId}");
        }
        if($("#flag").val()=='on'){
            $("#saveBtn").hide();
    ...
    function saveReport() {
        ...
        if($("#w_reportName").val()=="" || $("#w_reportName").val() =="0" || $("#w_reportName").val() == undefined){
            swal({
                title: "请填写报告名称！",
                type: "info"
            });
        ...
        if($("#w_reportDate").val()=="" || $("#w_reportDate").val() =="0" || $("#w_reportDate").val() == undefined){
            swal({
                title: "请选择报告时间！",
                type: "info"
            });
        ...
        if($("#w_consigner").val()=="" || $("#w_consigner").val() =="0" || $("#w_consigner").val() == undefined){
            swal({
                title: "请填写委托方！",
                type: "info"
            });
        ...
        showSaveLoading();
        $.post("<%=request.getContextPath()%>/teacherResult/saveTeachingResult/report", {
            id: pid,
            personId: headTList[1],
            deptId: headTList[0],
            reportName: $("#w_reportName").val(),
            reportDate: $("#w_reportDate").val(),
            typicalFlag: $("#w_typicalFlag").val(),
            role: $("#w_role").val(),
            consigner: $("#w_consigner").val(),
            remark: $("#w_remark").val()
        }, function (msg) {
            hideSaveLoading();
            if (msg.status == 1) {
