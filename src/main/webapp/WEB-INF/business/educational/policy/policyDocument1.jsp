        ...
        <div class="col-md-12">
            <div class="block">
                ...
                <div class="block block-drop-shadow content">
                    ...
                    <table id="policyDocumentGrid" cellpadding="0" cellspacing="0" width="100%"
...
<script>
    ...
    $(document).ready(function () {
        ...
        roleTable.on('click', 'tr a', function () {
            ...
            if (this.id == "uRole") {
                $("#dialog").load("<%=request.getContextPath()%>/policyDocument/addPolicyDocument?id=" + id +"&type=1");
                $("#dialog").modal("show");
            }
            if (this.id == "upload"){
            ...
            if (this.id == "delRole") {
                swal({
                    ...
                    $.post("<%=request.getContextPath()%>/policyDocument/deletePolicyDocumentById", {
                        id: id
                    }, function (msg) {
                        swal({
                            title: msg.msg,
    ...
    /*添加*/
    function addRewards() {
        $("#dialog").load("<%=request.getContextPath()%>/policyDocument/addPolicyDocument?type=1");
        $("#dialog").modal("show");
    }
    /*清空*/
    ...
    function searchAssets() {
        ...
        roleTable = $("#policyDocumentGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/policyDocument/policyDocumentAction',
                "data": {
            ...
            "columns": [
                ...
                { "data": "fileNumber"},
                { "render": function ()
                    {return "<a id='upload' class='icon-cloud-upload' title='上传附件'></a>"
                    }
                }
            ],
            "columnDefs": [{
