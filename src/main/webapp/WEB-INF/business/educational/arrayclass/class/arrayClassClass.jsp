...
<script>
            ...
            "columns": [
                ...
                {"width": "22%", "data": "remark", "title": "备注"},
                {
                    ...
                    "render": function (data, type, row) {
                        return '<span class="icon-th-list"title="设置班级信息" onclick=courseManage("' + row.arrayClassId + '","' + row.termValue + '","' + row.arrayClassName +'")/>';
                    }
                }
            ],
            'order' : [[1,'desc'],[2,'desc']],
    ...
    function courseManage(arrayClassId,termValue,arrayClassName) {
        $("#right").load("<%=request.getContextPath()%>/arrayClass/toArrayClassClassList?arrayClassId=" + arrayClassId+"&term=" + termValue +"&arrayClassName="+arrayClassName);
    }
        ...
        arrayClass = $("#arrayClassClassIdGrid").DataTable({
            ...
            "columns": [
                ...
                {"width": "22%", "data": "remark", "title": "备注"},
                {
                    ...
                    "title": "操作",
                    "render": function (data, type, row) {
                        return '<span class="icon-th-list" title="设置班级信息" onclick=courseManage("' + row.arrayClassId + '","' + row.termValue + '")/>';
                    }
                }
            ],
            'order' : [[1,'desc'],[2,'desc']],
