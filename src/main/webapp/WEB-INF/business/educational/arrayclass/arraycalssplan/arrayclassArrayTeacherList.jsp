...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <div class="block">
                <div class="header">
                    <span style="font-size: 15px;margin-left: 20px">${arrayClassName} > ${teacherPersonShow} > 设置教学计划</span>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        table = $("#table").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/arrayclassplan/getArrayclassArrayList?arrayclassId=${arrayclassId}&teacherPersonId=${teacherPersonId}',
            },
            "destroy": true,
            "columns": [
    ...
    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/arrayclassplan/toArrayclassArrayTeacherAdd?arrayclassTeacherId=${arrayclassTeacherId}")
        $("#dialog").modal("show")
    }
    ...
    function back() {
        $("#right").load("<%=request.getContextPath()%>/arrayclassplan/toArrayClassTecaherList?id=${arrayclassId}&arrayClassName=${arrayClassName}")
    }
</script>