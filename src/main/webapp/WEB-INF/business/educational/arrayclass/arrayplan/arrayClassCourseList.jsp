...
<script>
            ...
            "columns": [
                ...
                {"width": "22%", "data": "remark", "title": "备注"},
                {
                    ...
                    "render": function (data, type, row) {
                        return '<span class="icon-th-list"title="设置课程信息" onclick=courseManage("' + row.arrayClassId + '","' + row.termValue + '","' + row.arrayClassName +'")/>';
                    }
                }
            ],
            'order' : [[1,'desc'],[2,'desc']],
    ...
    function search() {
        ...
        arrayClass.ajax.url("<%=request.getContextPath()%>/arrayClass/getArrayClassList?arrayClassName="+t_arrayClassName+"&term="+t_term).load();
    }
</script>