...
<script>
    ...
    $("#studentNumber").blur(function () {
        if ($("#studentNumber").val() != "") {
            $.post("<%=request.getContextPath()%>/student/checkStudentNumber?studentNumber=" + $("#studentNumber").val(), function (data) {
                if ($("#studentId").val() == "") {
                    if (data.status > 0) {
    ...
    function updateEmp() {
        ...
        if (homePhone != "") {
            var telNum = /^1\d{10}$/;
            if (telNum.test(homePhone) === false) {
                swal({
                    title: "家庭电话不正确",
                    type: "info"
                });
