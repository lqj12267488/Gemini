...
<script>
    ...
    $(document).ready(function () {
        ...
        arrayClass.on('click', 'tr a', function () {
            ...
            if (this.id == "teacherList") {
                $("#right").load("<%=request.getContextPath()%>/arrayClassResultClass/teacherPersonalSyllabusView?arrayclassId=" + arrayclassId+"&arrayClassName="+arrayClassName);
            }
        });
    })
    ...
    function search() {
        ...
        arrayClass = $("#arrayClassGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/arrayClassResultClass/getArrayClassTeacherList',
                "data": {
                    arrayClassName:  arrayClassName,
                    term: term
                }
            ...
            "columns": [
                ...
                {"width": "22%", "data": "remark", "title": "备注"},
                {
                    ...
                    "title": "操作",
                    "render": function () {
                        return "<a id='teacherList' class='icon-th' title='教师个人课表查看'></a>";
                    }
                }
            ],
            'order' : [[1,'desc'],[2,'desc']],
