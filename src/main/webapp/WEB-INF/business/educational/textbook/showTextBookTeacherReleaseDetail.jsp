...
<script>
    ...
    $(document).ready(function () {
        ...
        table = $("#table").DataTable({
            "ajax": {
                "url": "<%=request.getContextPath()%>/textbook/getTextBookReleaseList?textbookId=" + textbookId_1  + "&submitState=2&personType=1",
            },
            "columns": [
                ...
                {"data": "receiver"},
                {
                    "render": function (data, type, row) {
                        var str = '<span class="icon-indent-right" title="发放" id="th" onclick=release("' + row.id + '","'+row.personType+'")/>&nbsp;&nbsp;&nbsp;'+
                            '<span id="edit" class="icon-edit" title="修改实订数量" onclick=edit("' + row.id + '","'+row.personType+'")/>'
                        return str;
                    },
                },
            ],
    ...
    function release(id,personType) {
        $("#textbookRelease").load("<%=request.getContextPath()%>/textbook/releaseTextbookById?id=" + id+"&personType="+personType);
        $("#textbookRelease").modal("show");
    }
    function edit(id,personType) {
        $("#textbookRelease").load("<%=request.getContextPath()%>/textbook/editActualNum?id=" + id+"&personType="+personType);
        $("#textbookRelease").modal("show");
    }
</script>
