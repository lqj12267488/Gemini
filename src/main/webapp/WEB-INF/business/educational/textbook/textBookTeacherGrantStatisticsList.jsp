...
<script>
    ...
    function exportTextBookStatisticsListAll() {
        window.location.href = "<%=request.getContextPath()%>/textbook/exportTextBookStatisticsListAll?textbookName=" + $("#t_textbookName").val() + "&personType=1";
    }
    function releaseTextbook(textbookId) {
        $("#right").load("<%=request.getContextPath()%>/textbook/textBookTeacherReleaseList?personType=${personType}&textbookId=" + textbookId );
        $("#right").modal("show");
    }
