...
<script>
                ...
                {"data": "majorShow", "title": "专业名称"},
                {
                    ...
                    "render": function (data, type, row) {
                        return '<span class="icon-edit" title="修改" onclick=edit("' + row.planId + '")/>&ensp;&ensp;' +
                            ...
                            '<a  href="<%=request.getContextPath()%>/courseplan/toExportPlan?planName=' + row.planName + '"><span class="icon-cloud-download" title="导出计划表" /></a>&ensp;&ensp;' +
    ...
    function add() {
        $("#dialog").load("<%=request.getContextPath()%>/courseplan/toAdd", {
            type: '${type}'
        })
        $("#dialog").modal("show")
    }
    ...
    function edit(id) {
        $("#dialog").load("<%=request.getContextPath()%>/courseplan/toEdit?id=" + id)
        $("#dialog").modal("show")
    }
