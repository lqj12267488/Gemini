...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <div class="block">
                <div class="header">
                    <span style="font-size: 15px;margin-left: 20px">${examName} >${roomName}> 设置考试科目</span>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        courseTable = $("#courseTableGrid").DataTable({
            "ajax": {
                "url": '<%=request.getContextPath()%>/exam/course/getExamCourseAllList?examId=${examId}&roomId=${roomId}',
            },
            "columns": [
                {
                ...
                {
                    "render": function () {
                        return "<a id='checkExamCourse' class='icon-flag-checkered' title='选择考试班级'></a>&nbsp;&nbsp;&nbsp;" +
        ...
        courseTable.on('click', 'tr a', function () {
            ...
            var examRoomId = data.examRoomId;
            var course = data.courseId;
            if (this.id == "updateSalary") {
                $("#dialog").load("<%=request.getContextPath()%>/exam/course/toAdd?examId=${examId}");
                $("#dialog").modal("show");
            }
            if (this.id == "checkExamCourse") {
                $("#dialog").load("<%=request.getContextPath()%>/exam/course/class/toAddExamRoomCourseExamClass?examId=${examId}&examCourseId="+id+"&examRoomId=${examRoomId}&roomId="+roomId+"&courseId="+course);
                $("#dialog").modal("show")
            }
    ...
    function addCourse() {
        $("#dialog").load("<%=request.getContextPath()%>/exam/course/toAddExamRoomCourse?examId=${examId}&examCourseId=${examCourseId}&examRoomId=${examRoomId}&roomId=${roomId}")
        $("#dialog").modal("show")
    }
    ...
    function searchclear() {
        ...
        $("#mid").val("");
        $("#cid").val("");
        courseTable.ajax.url("<%=request.getContextPath()%>/exam/course/getExamCourseAllList?examId="+$("#f_examId").val()+"&roomId=${roomId}").load();
    }
    ...
    function search() {
        ...
        courseTable.ajax.url("<%=request.getContextPath()%>/exam/course/getExamCourseAllList?departmentsId="+did+"&majorCode="+majorData+"&trainingLevel="+trainingLevel+"&courseId="+cid+"&examId=${examId}&roomId=${roomId}").load();
    ...
    function allot() {
        ...
        if ($('input[name="checkbox"]:checked').length > 0) {
            ...
            $("#dialog").load("<%=request.getContextPath()%>/exam/course/editExamCourseTime?ids=" + chk_value);
            $("#dialog").modal("show");
        } else {
    ...
    function back() {
        $("#right").load("<%=request.getContextPath()%>/exam/room/setExamRoom?examId=${examId}&examName=${examName}");
    }
</script>
