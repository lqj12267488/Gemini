...
<div class="modal-dialog">
    <div class="modal-content block-fill-white">
        <div class="modal-header">
            ...
            <input id="classId" hidden value="${courseConstr.classId}"/>
        </div>
        <div class="modal-body clearfix">
            ...
            <div class="controls">
                <div class="form-row">
                    ...
                    <div class="col-md-9">
                        <input  type="text" id="className" name="className" value="${courseConstr.className}"
                                onKeypress="javascript:if(event.keyCode == 32)event.returnValue = false; "maxlength="20" placeholder="最多输入20个数字" />
                    </div>
                </div>
                <div class="form-row">
                    <div class="col-md-3 tar">
                ...
                <div class="form-row">
                    ...
                    <div class="col-md-9">
                        <input  type="text" id="prescribedPeriods" name="prescribedPeriods" value="${courseConstr.prescribedPeriods}"
                ...
                <div class="form-row">
                    ...
                    <div class="col-md-9">
                        <input  type="text" id="practiceClass" name="practiceClass" value="${courseConstr.practiceClass}"
                                onKeypress="javascript:if(event.keyCode == 32)event.returnValue = false; "maxlength="2" placeholder="最多输入2个数字" />
                    </div>
                </div>
                <div class="form-row">
...
<script>
    ...
    $(document).ready(function () {
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=ZYSKFS", function (data) {
            addOption(data, 'teachingMethod', '${courseConstr.teachingMethod}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=KCJSLX", function (data) {
            addOption(data, 'courseTypes', '${courseConstr.classTypes}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=KCJSSX", function (data) {
            addOption(data, 'courseProperties', '${courseConstr.courseProperties}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=KCJPKC", function (data) {
            addOption(data, 'excellentCourse', '${courseConstr.excellentCourse}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=KCSKDD", function (data) {
            addOption(data, 'venue', '${courseConstr.venue}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=KCKSKHFF", function (data) {
            addOption(data, 'testMethod', '${courseConstr.testMethod}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=KCSF", function (data) {
            addOption(data, 'classCertifiate', '${courseConstr.classCertifiate}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=KCSF", function (data) {
            addOption(data, 'coreCurriculum', '${courseConstr.coreCurriculum}');
        });
        $.get("<%=request.getContextPath()%>/common/getSysDict?name=KCSF", function (data) {
            addOption(data, 'schoolEnterpriseCooperation', '${courseConstr.schoolEnterpriseCooperation}');
        });
    ...
    function save() {
        ...
        $.post("<%=request.getContextPath()%>/courseconstr/checkCourseConstrMajorName",{
            classId: classId,
            className: className,
        },function(msg1) {
            if (msg1.status == 1) {
                ...
                showSaveLoading();
                $.post("<%=request.getContextPath()%>/courseconstr/saveMajor", {
                    classId: $("#classId").val(),
                    classTypes: $("#courseTypes option:selected").val(),
