...
<script>
    ...
    $(document).ready(function () {
        ...
        courseTable.on('click', 'tr a', function () {
            ...
            var id = data.id;
            var chid = data.teacherId;
            if (this.id == "uHonor") {
                $("#dialog").load("<%=request.getContextPath()%>/teacherhonor/editTeacherhonorManageById?id=" + id);
                $("#dialog").modal("show");
            }
            if (this.id == "upload") {
            ...
            if (this.id == "delRole") {
                swal({
                    ...
                    $.get("<%=request.getContextPath()%>/teacherhonor/deleteTeacherhonorManageById?id=" + id, function (msg) {
                        if (msg.status == 1) {
                            swal({
                                title: msg.msg,
                                type: "success"
    ...
    function addCoursehonor() {
        $("#dialog").load("<%=request.getContextPath()%>/teacherhonor/addTeacherhonorManage?id=" + $("#teacherId").val());
    ...
    function search() {
        courseTable = $("#coursehonor").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/teacherhonor/getTeacherhonorManagelList',
                "data": {
            ...
            "columns": [
                ...
                {"width": "12%", "data": "honorLevel", "title": "荣誉级别"},
                {
                    "width": "12%",
                    "title": "操作",
                    "render": function () {
