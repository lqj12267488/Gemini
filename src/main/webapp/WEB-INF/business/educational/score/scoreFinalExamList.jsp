...
<script>
    ...
    function search() {
        ...
        listTable = $("#listGrid").DataTable({
            ...
            "columns": [
                ...
                {
                    "width": "6%", "title": "考试成绩", "data": "examScore",
                    "render": function (data, type, row) {
                        if (row.submitFlag == '1') {
                        ...
                        if (row.examScore != null) {
                            scoreIn = row.examScore;
                        }
                        var html = "<input name='examScore_" + row.id + "' value='" + scoreIn + "' maxlength='10'>";
                        return html;
                    }
                },

                {"data": "score", "width": "6%", "title": "期末成绩"},
    ...
    //导入
    function impdata() {
        $("#dialog").load("<%=request.getContextPath()%>/scoreMakeup/scoreMakeupImport?type=${type}&scoreExamId=${scoreExamId}&scoreExamName=" + $("#scoreExamName").val()+"&normalType=${normalType}"+"&courseId=${courseId}");
        $("#dialog").modal("show");
    }
