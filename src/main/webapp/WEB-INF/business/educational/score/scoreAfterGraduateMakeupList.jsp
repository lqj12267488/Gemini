...
<script>
    ...
    function search() {
        ...
        listTable = $("#listGrid").DataTable({
            "ajax": {
                "type": "post",
                "url": '<%=request.getContextPath()%>/scoreMakeup/getScoreAfterGraduateMakeupList',
                "data": {
            ...
            "columns": [
                ...
                {
                    "data": "afertGraduateMakeupStatus",
                    "render": function (data, type, row) {
                        var html = "<select name='" + row.id + "'>";
                        $.each(dataS, function (index, content) {
                            if(content.id !='4')
                                if (content.text === row.afertGraduateMakeupStatus) {
                ...
                },
                {
                    ...
                    "render": function (data, type, row) {
                        ...
                        $.each(dataBKCJZT, function (index, content) {
                            if (content.text == row.afertGraduateMakeupScore ||
                                    content.id == row.afertGraduateMakeupScore ) {
    ...
    //导出
    function expdata() {
        var href = "<%=request.getContextPath()%>/scoreMakeup/exportMakeupScore?type=3&scoreExamId=${scoreExamId}&scoreExamName=${scoreExamName}" +
    ...
    function saveScore() {
        ...
        $.post("<%=request.getContextPath()%>/scoreAfterGraduateMakeup/saveScore", scores, function (msg) {
